...
- 追加前若 `活动段大小 + 帧长 > SEGMENT_MAX`,直接开新段;链跨段延续
  (记录的 `seq` 与 `prev` 与它落在哪个文件无关)。**轮转发生在命令之间,
  不在命令内部**——一条命令的记录留在同一段里,于是撕裂尾部最多只能吃掉
  一条命令的尾巴。
- `SEGMENT_MAX` 是旧值锚意义上的可调数值(建议 4 MiB),**不是契约**。

**不写封段记录**(修订,原设计有 `segment_sealed` 帧)。两条理由:长度前缀
帧已经证明了一个段是否干净结束(尾部判定为 `none`),封段帧对完整性是冗余的;
而它原本要携带的 `state_digest` **必须删掉**——那是把某一个调用方的投影烘焙
进持久链,内核就不再是领域无关的了,同一个 store 换个 reducer 打开就会与链
自相矛盾。代价是快照失去了链内锚点,故快照只能是缓存(见 §5 P2/P9)。
...
| P2b | 快照是缓存不是权威 | 快照与链的绑定(seq + head digest)不符时**忽略**它并全量重放,而不是失败关闭——一个能让 store 打不开的"可弃缓存"不叫可弃 | LK-12 |
...
快照存 `{store_id, seq, head_digest, state, digest}`,自带 digest 防文件损坏,
并以 `seq + head_digest` 绑定到链上的一个具体位置。但**快照的 state 本身不是
链可证的**——链里没有、也不该有任何一个调用方的投影(见 §2)。因此:

- 绑定不符、digest 不符、store_id 不符 → **忽略该快照**,全量重放(P2b)。
- 一个绑定正确而 state 被篡改的快照,不会被自动识破。这是快照作为缓存的固有
  代价,不是可以靠加字段消除的疏漏。
...
- 2026-07-25:删除封段记录(§2)。原设计让它携带 `state_digest`,那会把某个
  调用方的投影烘焙进持久链,内核就不再领域无关;而完整性上它对长度前缀帧是
  冗余的。连带后果:快照失去链内锚点,只能是缓存(§5.1),C7 退化为等同 C1。
