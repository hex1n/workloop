...
2. **锁内重查幂等**。锁外那次只是省事;真正防止"重试与原调用交错各提交一次"的是锁内
   这次。〔评审后补测〕此前记为"无法确定性构造",做法是**等竞态自己发生**;换成
   **把那一刻命名出来**(`onPhase("git_index_locked")`,与切片 1 崩溃矩阵同一手法)
   之后它是可测的。关键是测试里必须让**工作树在两个调用者之间再动一次**——否则
   "为已有提交作证"那条路径会把竞态吸收掉,两条路给出同一个答案,守卫看起来没用。
   现在去掉锁内复检,会多出一个**账本没有的提交**,测试变红。
3. **写入时不再检查 liveness**。若循环在 git 操作期间被终结,提交已经发生了,拒绝
   记录只会让仓库持有一个账本否认的变更。**收据是关于 git 的事实,不是循环的状态转移。**

**剩余缺口**:进程若恰好死在 git 提交与账本追加之间,仓库里会留下一个账本没有的
提交。这个窗口**关不掉**——git 提交先于任何关于它的记录存在,两者不在一个原子
边界内。切片 1 的日志内核对自己的写入有崩溃矩阵,但它管不到另一个系统的状态机。

后续处置见 [09-reconciliation](09-reconciliation.md):提交带 trailer 记下它属于
哪个循环、由哪条命令产生,`status` 因此能**逐个指名**这样的提交;同 command id
重跑 `receipt` 会走 §3.1 那条"为已有提交作证"的路径把缺口补上。**自动修复不做**
——那要改写 git 历史,是执行,归宿主。
