...
function fixture(t, name) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `workloop-current-${name}-`));
...
test("current Git tracer selects containment and replays after disposable projections are removed", (t) => {
  ...
  for (const target of targets) {
    const status = json(run(["current-status", "--target", target], { cwd: fx.root }));
  ...
  const beforeAudit = json(run(["current-audit", "--target", targets[0]], { cwd: fx.root }));
  ...
  const rebuilt = json(run(["current-status", "--target", targets[2]], { cwd: fx.root }));
...
test("current Git Hook receipts are target-routed and default nudge stays nonblocking", (t) => {
  ...
  const post = run(["current-hook", "--profile", "codex-safe", "--mode", "nudge"], { cwd: fx.root, input: payload("PostToolUse", { success: true }) });
  assert.equal(post.status, 0, post.stderr);
  assert.equal(post.stdout, "");
  const ledger = json(run(["current-ledger", "--target", target], { cwd: fx.root })).records;
...
test("current open requires replayable command provenance and disposable projection failures degrade open", (t) => {
  ...
  const sequence = json(run(["current-ledger", "--target", replay.repo])).records.length;
  ...
  assert.equal(json(run(["current-ledger", "--target", replay.repo])).records.length, sequence);
...
test("target-first routing covers canonical aliases, case, nesting, external and control targets", (t) => {
  ...
  assert.equal(json(run(["current-status", "--target", path.join(alias, "tracked.txt")])).authority_id, opened.authority_id);

  const differentlyCased = path.join(fx.repo.toUpperCase(), "SRC", "TRACKED.TXT");
  const caseResult = run(["current-status", "--target", differentlyCased]);
  ...
  const external = run(["current-status", "--target", path.join(fx.root, "outside.txt")]);
  ...
  for (const control of [path.join(fx.gitDir, "config"), path.join(fx.repo, ".workloop", "private"), opened.outcome_path]) {
    const result = run(["current-status", "--target", control]);
  ...
  fs.mkdirSync(nested);
  git(nested, ["init", "-q"]);
  const nestedStatus = json(run(["current-status", "--target", nested]));
...
test("persisted schema and task-engine transitions reject hash-valid invalid authority", (t) => {
  ...
  const ledger = json(run(["current-ledger", "--target", transitionFx.repo])).records;
  ...
  const rejectedTransition = run(["current-status", "--target", transitionFx.repo]);
  ...
  const schemaLedger = json(run(["current-ledger", "--target", schemaFx.repo])).records;
  ...
  const rejectedSchema = run(["current-status", "--target", schemaFx.repo]);
  ...
  const duplicateLedger = json(run(["current-ledger", "--target", duplicateFx.repo])).records;
  ...
  const rejectedDuplicate = run(["current-status", "--target", duplicateFx.repo]);
  ...
  const status = json(run(["current-status", "--target", locatorFx.repo]));
  const audit = json(run(["current-audit", "--target", locatorFx.repo]));
  assert.equal(status.routable, false);
