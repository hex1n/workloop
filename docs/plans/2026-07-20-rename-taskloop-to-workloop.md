...
3. [x] `WORKLOOP_INSTALL_HOME` 临时家下的全新安装干净。**原文写的"安装→卸载
   往返"当时不可执行**:项目没有安装器卸载入口,`uninstall` 只存在于
   `hooks --action record-uninstall`(纯留痕)。该缺口已于 2026-07-21 补上
   (`uninstall.mjs`,见 AGENTS.md),往返判据现已可按字面执行并通过。
   下列为 2026-07-20 先行验证的安装侧,全部通过:
