...
const WRITE_TOOL_MATCHER = "apply_patch|Write|Edit|MultiEdit|Bash|PowerShell|mcp__.*";
...
function hostCompletionOutcome(event, source) {
  ...
  if (exitCode === 0 || response.success === true || response.is_error === false) return "success";
...
function decodeHook({ profile, payload }) {
  ...
  return {
    ...
    ...(completionEvent ? {
      completionOutcome: hostCompletionOutcome(event, source),
...
function buildHookRecipe({ profile, command, mode = "nudge" }) {
  ...
  const preStatus = mode === "deny" ? "Enforcing workloop policy" : "Observing workloop operation intent";
  const stopStatus = mode === "deny" && !codex ? "Checking workloop completion state" : "Recording workloop stop census";
  return {
    hooks: {
      PreToolUse: [{ matcher: PRE_TOOL_USE_MATCHER, hooks: [{ type: "command", command: handler, ...(codex ? { statusMessage: preStatus } : {}), timeout: PRE_TOOL_USE_RECIPE_TIMEOUT_SECONDS }] }],
      ...
      Stop: [{ matcher: STOP_MATCHER, hooks: [{ type: "command", command: handler, ...(codex ? { statusMessage: stopStatus } : {}), timeout: STOP_RECIPE_TIMEOUT_SECONDS }] }],
    },
  };
}
...
function encodeHook({ invocation, disposition }) {
  ...
  const enforcementMode = (invocation?.mode ?? "nudge") === "deny";
  ...
  if (disposition?.event === "pre_tool_use" && disposition.action === "deny") {
    if (!enforcementMode) return encoded();
  ...
  if (disposition?.event === "pre_tool_use" && disposition.action === "rewrite") {
    const hookSpecificOutput = enforcementMode
      ? { hookEventName: "PreToolUse", permissionDecision: "allow", updatedInput: disposition.updatedInput }
      : { hookEventName: "PreToolUse", updatedInput: disposition.updatedInput };
    return encoded(JSON.stringify({ hookSpecificOutput }) + "\n");
  }
