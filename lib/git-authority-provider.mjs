...
function receiptInput(input, action) { if (!nonempty(input?.target) || !nonempty(input?.taskId) || !nonempty(input?.commandId) || input.grantedBy !== "user" || !nonempty(input.reason) || (action === "commit" && (!nonempty(input.message) || input.message.length > 4096))) throw providerError("GIT_RECEIPT_AUTHORIZATION_REQUIRED", "Git receipt operations require target, task, command identity, user provenance, reason, and commit message"); return { ...input, taskId: input.taskId.trim(), commandId: input.commandId.trim(), reason: input.reason.trim(), message: input.message?.trim() ?? null, sessionId: nonempty(input.sessionId) ? input.sessionId.trim() : "cli" }; }
function gitIndexDigest(discovery) { return sha256Hex(gitOutput(discovery.worktree_root, ["write-tree"])); }
function gitCachedPaths(discovery) { const output = gitCommand(discovery.worktree_root, ["diff", "--cached", "--name-only"], { allowFailure: false }).stdout.trim(); return output ? output.split(/\r?\n/u).filter(Boolean).sort(compareCodeUnits) : []; }
function gitTaskPaths(task) { return task.write_claims.map((claim) => claim.path === "." ? "." : claim.path); }
function taskOwnsPaths(task, paths) { return paths.every((item) => task.write_claims.some((claim) => claimContains(claim, item))); }
function hasCleanStageReceipt(task, indexDigest, headOid) { return task.git_receipts.some((receipt) => receipt.kind === "git_stage_receipted" && receipt.clean === true && receipt.index_after === indexDigest && receipt.head_oid === headOid); }
function receiptTask(discovery, records, input, evolveAllCurrentAuthority) { const state = evolveAllCurrentAuthority(null, records); const value = project(discovery, evolveAllCurrentAuthority, records, { taskId: input.taskId, sessionId: input.sessionId }); const task = state?.tasks.find((item) => item.task_id === input.taskId) ?? null; if (!task || task.attachment_id !== value.attachment_id || value.attachment?.availability !== "available" || !task.participant_session_ids.includes(input.sessionId)) throw providerError("TASK_ATTACHMENT_MISMATCH", "Git receipt target does not own an available participant task"); return { state, value, task }; }
function currentGitReceipt(input, action, runtime) { const normalized = receiptInput(input, action); const discovery = resolveGitAuthorityTarget(normalized.target); const lockManager = lockManagerFor(discovery, runtime.createLockManager); let receipt; lockManager.withLock("git_operation", discovery.git_dir, () => { let facts; if (action === "stage") { const beforePaths = gitCachedPaths(discovery); const indexBefore = gitIndexDigest(discovery); const headBefore = gitOutput(discovery.worktree_root, ["rev-parse", "HEAD"]); let task; lockManager.withLock("authority", discovery.authority_root, () => { const records = readAuthority(discovery, runtime.evolveAllCurrentAuthority); task = receiptTask(discovery, records, normalized, runtime.evolveAllCurrentAuthority).task; }); gitCommand(discovery.worktree_root, ["add", "--", ...gitTaskPaths(task)]); const afterPaths = gitCachedPaths(discovery); const indexAfter = gitIndexDigest(discovery); const headAfter = gitOutput(discovery.worktree_root, ["rev-parse", "HEAD"]); const paths = afterPaths; const clean = beforePaths.length === 0 && headBefore === headAfter && taskOwnsPaths(task, afterPaths); facts = { task, payload: { task_id: task.task_id, attachment_id: task.attachment_id, head_oid: headAfter, index_before: indexBefore, index_after: indexAfter, paths, clean, status: clean ? "clean" : "uncertain", granted_by: normalized.grantedBy, reason: normalized.reason } }; } else { const beforePaths = gitCachedPaths(discovery); const indexBefore = gitIndexDigest(discovery); let task; lockManager.withLock("authority", discovery.authority_root, () => { const records = readAuthority(discovery, runtime.evolveAllCurrentAuthority); task = receiptTask(discovery, records, normalized, runtime.evolveAllCurrentAuthority).task; }); const priorHead = gitOutput(discovery.worktree_root, ["rev-parse", "HEAD"]); gitCommand(discovery.worktree_root, ["commit", "--only", "--no-verify", "-m", normalized.message, "--", ...gitTaskPaths(task)]); const commitOid = gitOutput(discovery.worktree_root, ["rev-parse", "HEAD"]); const parents = gitOutput(discovery.worktree_root, ["rev-list", "--parents", "-n", "1", commitOid]).split(" "); const diffOutput = gitCommand(discovery.worktree_root, ["diff-tree", "--no-commit-id", "--name-only", "-r", commitOid]).stdout.trim(); const diffPaths = diffOutput ? diffOutput.split(/\r?\n/u).filter(Boolean).sort(compareCodeUnits) : []; const indexAfter = gitIndexDigest(discovery); const afterPaths = gitCachedPaths(discovery); const clean = hasCleanStageReceipt(task, indexBefore, priorHead) && parents[1] === priorHead && taskOwnsPaths(task, diffPaths) && afterPaths.length === 0; facts = { task, payload: { task_id: task.task_id, attachment_id: task.attachment_id, prior_head: priorHead, commit_oid: commitOid, parent_oid: parents[1] ?? "", diff_paths: diffPaths, index_after: indexAfter, clean, status: clean ? "clean" : "uncertain", granted_by: normalized.grantedBy, reason: normalized.reason } }; } lockManager.withLock("authority", discovery.authority_root, () => { const records = readAuthority(discovery, runtime.evolveAllCurrentAuthority); receiptTask(discovery, records, normalized, runtime.evolveAllCurrentAuthority); const kind = action === "stage" ? "git_stage_receipted" : "git_commit_receipted"; const record = appendAuthorityRecord(discovery, records, { commandId: normalized.commandId, kind, payload: facts.payload }, runtime.evolveAllCurrentAuthority); receipt = { ...facts.payload, command_id: normalized.commandId, authority_sequence: record.sequence, receipt_digest: record.record_digest }; writeCurrentSnapshot(discovery, project(discovery, runtime.evolveAllCurrentAuthority, records, { taskId: normalized.taskId, sessionId: normalized.sessionId })); }); }); const value = project(discovery, runtime.evolveAllCurrentAuthority, null, { taskId: normalized.taskId, sessionId: normalized.sessionId }); return { ...value, receipt, ...storageFields(discovery, value.authority_id) }; }
function stageCurrentGitTask(input, runtime) { return currentGitReceipt(input, "stage", runtime); }
function commitCurrentGitTask(input, runtime) { return currentGitReceipt(input, "commit", runtime); }

function certificationInput(input) { if (!nonempty(input?.target) || !nonempty(input?.taskId) || !nonempty(input?.commandId) || !nonempty(input?.sessionId) || input.sessionId === "cli" || !new Set(["self", "user"]).has(input.grantedBy) || !nonempty(input.reason)) throw providerError("INVALID_CERTIFICATION", "current certification requires target, task, command, participant session, provenance, and reason"); return { ...input, taskId: input.taskId.trim(), commandId: input.commandId.trim(), sessionId: input.sessionId.trim(), reason: input.reason.trim() }; }
function cleanCommitReceipt(task) { return [...task.git_receipts].reverse().find((receipt) => receipt.kind === "git_commit_receipted" && receipt.clean === true) ?? null; }
function prepareCurrentGitCertification(input, runtime) { const normalized = certificationInput(input); const discovery = resolveGitAuthorityTarget(normalized.target); const locks = lockManagerFor(discovery, runtime.createLockManager); let prepared; locks.withLock("authority", discovery.authority_root, () => { const records = readAuthority(discovery, runtime.evolveAllCurrentAuthority); const value = project(discovery, runtime.evolveAllCurrentAuthority, records, { taskId: normalized.taskId, sessionId: normalized.sessionId }); const state = runtime.evolveAllCurrentAuthority(null, records); const task = state?.tasks.find((item) => item.task_id === normalized.taskId) ?? null; const receipt = task ? cleanCommitReceipt(task) : null; if (!value.routable || !task || value.attachment?.availability !== "available" || !receipt) throw providerError("CERTIFICATION_PRECONDITION_FAILED", "Git certification requires an available active task with a clean commit receipt"); prepared = { provider: "git_common", authority_id: state.authority_id, task_id: task.task_id, attachment_id: task.attachment_id, prepared_sequence: records.length, attachment_final_digest: task.attachment_final_digest, criterion_root: discovery.worktree_root, commit_oid: receipt.commit_oid }; }); return prepared; }
function certifyCurrentGitTask(input, prepared, criterionDigest, runtime) { const normalized = certificationInput(input); const discovery = resolveGitAuthorityTarget(normalized.target); const locks = lockManagerFor(discovery, runtime.createLockManager); let value; locks.withLock("git_operation", discovery.git_dir, () => { locks.withLock("authority", discovery.authority_root, () => { const records = readAuthority(discovery, runtime.evolveAllCurrentAuthority); const state = runtime.evolveAllCurrentAuthority(null, records); const current = project(discovery, runtime.evolveAllCurrentAuthority, records, { taskId: normalized.taskId, sessionId: normalized.sessionId }); const task = state?.tasks.find((item) => item.task_id === normalized.taskId) ?? null; if (!current.routable || current.attachment?.availability !== "available" || !task || state.authority_id !== prepared.authority_id || task.task_id !== prepared.task_id || task.attachment_id !== prepared.attachment_id || current.attachment_id !== prepared.attachment_id || records.length !== prepared.prepared_sequence || task.attachment_final_digest !== prepared.attachment_final_digest) throw providerError("CERTIFICATION_STALE", "authority, attachment, or task changed during criterion execution"); if (gitCommand(discovery.worktree_root, ["merge-base", "--is-ancestor", prepared.commit_oid, "HEAD"], { allowFailure: true }).status !== 0) throw providerError("CERTIFICATION_RECEIPT_UNLANDED", "clean receipt commit is no longer landed on HEAD"); if (gitCommand(discovery.worktree_root,  ["diff", "--quiet", prepared.commit_oid, "--", ...gitTaskPaths(task)], { allowFailure: true }).status !== 0) throw providerError("CERTIFICATION_PATHS_DRIFTED", "task paths changed after the clean receipt commit"); appendAuthorityRecord(discovery, records, { commandId: normalized.commandId, kind: "task_certified", payload: { task_id: task.task_id, attachment_id: task.attachment_id, session_id: normalized.sessionId, prepared_sequence: prepared.prepared_sequence, attachment_final_digest: task.attachment_final_digest, criterion_digest: criterionDigest, commit_oid: prepared.commit_oid, reason: normalized.reason, granted_by: normalized.grantedBy } }, runtime.evolveAllCurrentAuthority); value = project(discovery, runtime.evolveAllCurrentAuthority, records, { taskId: normalized.taskId, sessionId: normalized.sessionId }); writeCurrentSnapshot(discovery, value); }); }); return { ...value, certification: value.task?.certification ?? null, ...storageFields(discovery, value.authority_id) }; }

export { mutateCurrentGitTask, openCurrentGitTask, queryCurrentGit, recordCurrentGitHook, recoverCurrentGitAttachment, forkCurrentGitIdentity, resolveGitAuthorityTarget, stageCurrentGitTask, commitCurrentGitTask, prepareCurrentGitCertification, certifyCurrentGitTask };
