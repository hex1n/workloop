...
function prepareOpen(discovery, input, evolveAllCurrentAuthority) {
  ...
  if (!intent) {
    const anchored = state.attachments.find((item) => item.anchor_id === discovery.anchor_id) ?? null;
    if (anchored && state.tasks.some((item) => item.attachment_id === anchored.attachment_id)) throw providerError("TASK_ALREADY_OPEN", "Ticket 04 permits one partitioned task per Git attachment");
    if (anchored) throw providerError("RECOVERY_REQUIRED", "the current Git attachment exists without a completed task; explicit recovery is required");
...
function unavailableAttachmentView(attachment, task, unavailableReason) {
  return { attachment_id: attachment.attachment_id, lifecycle: attachment.lifecycle, availability: "unavailable", unavailable_reason: unavailableReason, claimed_worktree_root: attachment.worktree_root, observed_worktree_root: null, path_status: "unavailable", task };
}

function observeAttachment(attachment, authorityId, task, currentDiscovery = null) {
  ...
  catch (error) { return unavailableAttachmentView(attachment, task, error?.code === "ENOENT" ? "git_admin_unavailable" : "git_admin_unreadable"); }
  if (observedAnchor !== attachment.anchor_id) return unavailableAttachmentView(attachment, task, "anchor_mismatch");
  let locator;
  try { locator = readLocator({ attachment_root: observedGitDir }).at(-1) ?? null; }
  catch { return unavailableAttachmentView(attachment, task, "locator_unreadable"); }
  ...
  if (!locatorMatches) return unavailableAttachmentView(attachment, task, "locator_unavailable");
  let observed;
  try { observed = currentAnchor?.worktree_root ?? observedWorktreeRoot(attachment); }
  catch { return unavailableAttachmentView(attachment, task, "worktree_unavailable"); }
  return { attachment_id: attachment.attachment_id, lifecycle: attachment.lifecycle, availability: "available", unavailable_reason: null, claimed_worktree_root: attachment.worktree_root, observed_worktree_root: observed, path_status: path.relative(attachment.worktree_root, observed) === "" ? "unchanged" : "moved", task };
}
...
function project(discovery, evolveAllCurrentAuthority, suppliedRecords = null) {
  ...
  const repositoryTasks = state ? state.attachments.map((item) => observeAttachment(item, state.authority_id, state.tasks.find((task) => task.attachment_id === item.attachment_id) ?? null, discovery)) : [];
  ...
  const attachmentView = attachment ? repositoryTasks.find((item) => item.attachment_id === attachment.attachment_id) ?? null : null;
  const task = attachment && state ? state.tasks.find((item) => item.attachment_id === attachment.attachment_id) ?? null : null;
  ...
  return { provider: "git_common", authority_id: state?.authority_id ?? null, attachment_id: attachment?.attachment_id ?? null, placement: task?.placement ?? attachment?.placement ?? "partitioned", routable, routing_reason: reason, authority_sequence: records.length, attachment: attachmentView, task, repository_tasks: repositoryTasks };
}
...
function queryCurrentGit(target, kind = "status", { createLockManager, evolveAllCurrentAuthority }) {
  ...
  if (kind === "tasks") return { provider: value.provider, authority_id: value.authority_id, authority_sequence: value.authority_sequence, repository_tasks: value.repository_tasks, warnings, ...storageFields(discovery, value.authority_id) };
  return { ...value, warnings, ...storageFields(discovery, value.authority_id) };
}
...
function recordCurrentGitHook({ target, invocation }, { createLockManager, runAuthorityTransaction, evolveAllCurrentAuthority }) {
  ...
  const commandId = `hook:${before.attachment_id}:${before.task.task_id}:${invocation.event}:${operationId}`;
  runAuthorityTransaction({
    lockManager,
    authorityId: discovery.authority_root,
    append: () => {
