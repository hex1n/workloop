...
import { EXPLICIT_PROFILES, POST_TOOL_USE_RECIPE_TIMEOUT_SECONDS, STOP_INLINE_CRITERION_SECONDS, STOP_RUNTIME_DEADLINE_SECONDS, buildHookRecipe, decodeHook, encodeHook, hostProfileCapability } from "./host-hooks.mjs";
...
function historyRequirementFrom(values) {
  ...
  return values.risk === "critical" || values.writes !== undefined ? "complete" : "artifact_only";
}
  ...
  if (current.task.runtime_contract === 6) {
    ...
    if (checkpoint.checkpoint_id !== current.task.artifact_checkpoint.checkpoint_id) {
      ...
      commitTaskCommand(repo, {
        ...
        currentScopeViolations: delta.changed_paths.filter((entryPath) => !insideEnvelope(entryPath, current.task.envelope.files)),
        coverage: "full",
    ...
    if (!authorityMatches) {
      ...
      return {
        ...
        evidence: sideEffect && recordSideEffect && sameMutableTask
          ? action({ authority, task: current, authorityChanged: true, repositorySnapshot: currentSnapshot })
          : null,
      };
    }
    if (sideEffect) {
      ...
      return { status: "stale", evidence: recordSideEffect ? action({ authority, task: current, authorityChanged: false, repositorySnapshot: currentSnapshot }) : null };
    }
...
function cmdVerifyRecord(repo) {
  ...
  if (executed.status === "busy") return error(criterionInProgressMessage(executed.lease));
  const observation = executed.observation;
  const committed = commitPreparedObservation(repo, prepared, executed, ({ authority, task }) => {
    const atEpochMs = Date.now();
    const at = utcTimestamp(atEpochMs);
    observation.observed_at = at;
    if (observation.execution.execution_error === "criterion_side_effect") {
      return { kind: "side_effect", committed: commitTaskCommand(repo, { type: "criterion-side-effect", taskId: task.task_id, observation, actingSession: actingSessionId(), at, atEpochMs }, { authority }) };
    }
...
function cmdVerify(values) {
  ...
  const finalized = commitPreparedObservation(repo, prepared, executed, ({ authority, task }) => {
    if (observation.execution.execution_error !== "criterion_side_effect" || task.lifecycle.state === "terminal") return null;
    const atEpochMs = Date.now(); const at = utcTimestamp(atEpochMs);
    observation.observed_at = at;
    return commitTaskCommand(repo, { type: "criterion-side-effect", taskId: task.task_id, observation, actingSession: hostSessionId(), at, atEpochMs }, { authority });
  });
...
function adjudicateClosure(repo, { explicit, stop, sessionId, actorSession = null, transcriptPath, inlineCriterionBudgetSeconds = null }, readiness) {
  ...
  const observation = executed.observation;
  const finalized = commitPreparedObservation(repo, prepared, executed, ({ authority, task, authorityChanged = false }) => {
    const atEpochMs = Date.now(); const at = utcTimestamp(atEpochMs);
    observation.observed_at = at;
    if (observation.execution.execution_error === "criterion_side_effect") {
      return {
        kind: "side_effect",
        committed: commitTaskCommand(repo, { type: "criterion-side-effect", taskId: task.task_id, observation, transcriptRange: authorityChanged ? null : prepared.transcript_range, actingSession: stop ? sessionId : hostSessionId(), at, atEpochMs }, { actorKind: stop ? "hook" : "cli", authority }),
      };
    }
    const drift = criterionDrift(task.criterion, repo);
...
function hookPretool(invocation, repo, callAnalysis) {
  ...
  try { return withTaskLock(repo, () => {
    ...
    if (current.runtime_contract === 6 && current.history_requirement === "complete" && !hostCapability.exhaustive_surface) {
      return persistDeny(`complete mutation history is unavailable for ${invocation.profile}; this host surface is non-exhaustive, so reopen or amend with --history-requirement artifact-only only if partial history is acceptable`);
    }
...
function hookPosttool(invocation, repo, callAnalysis) {
  ...
  const deadlineEpochMs = Date.now() + POST_TOOL_USE_RECIPE_TIMEOUT_SECONDS * 1000;
  ...
  return withTaskLock(repo, () => {
    ...
    const validation = validateRepoSnapshot(repo, repositorySnapshot, { deadlineEpochMs: Math.min(deadlineEpochMs, Date.now() + OBSERVATION_COMMIT_VALIDATION_MS) });
    if (!validation.matches) throw new Error("PostToolUse repository snapshot changed before receipt commit");
    ...
    const mutationHistory = !authorized ? "unknown"
    ...
    const prewriteEnforcement = !authorized ? "unknown"
    ...
    commitTaskCommand(repo, {
      ...
      checkpointId: checkpoint.checkpoint_id, fromCheckpoint: current.artifact_checkpoint.checkpoint_id,
      toCheckpoint: checkpoint.checkpoint_id, changedEntries: delta.changed_entries, changedPaths: delta.changed_paths,
      currentScopeViolations: delta.changed_paths.filter((entryPath) => !insideEnvelope(entryPath, current.envelope.files)),
      coverage: "full", reason: event === "post_tool_use_failure" ? "PostToolUseFailure reconciliation" : "PostToolUse reconciliation",
      coverageChange: {
        artifactState: "full", mutationHistory, prewriteEnforcement,
        ...
        reason: authorized ? "operation completion receipt" : "orphan operation completion receipt",
      },
...
function dispatchHook(profile = "unknown", mode = "deny") {
  ...
  try {
    ...
    if (new Set(["post_tool_use", "post_tool_use_failure"]).has(event)) {
      process.stderr.write(`workloop: completion receipt degraded: ${err?.message ?? err}\n`);
