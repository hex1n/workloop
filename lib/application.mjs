...
import { commandExecutionViews, commandSafetyFailure, commandShapes, commandValues, controlPlaneWriteFailure, envelopeDirty, envelopeOverlap, foreignWriteDecision, gitOps, insideEnvelope, joinedFileOffender, joinedFilesMessage, looksLikeWrite, siblingWorktreeOpenTasks, warnZeroMatchEnvelope, writeFileTargets } from "./supervision.mjs";
...
const WRITE_COMMAND_SHAPES = new Set(["publish", "git_push", "destructive", "install", "network_write"]);
...
function hasCommandWriteTarget(tool, mapping) {
  return commandValues(mapping).length > 0 && writeFileTargets(tool, mapping).length > 0;
}
...
function taskloopCommandCandidates(command, tool) {
  const raw = String(command);
  const nested = /powershell/i.test(tool) ? [] : commandExecutionViews(raw).slice(1);
  return [raw, ...nested];
}
function taskloopInvocations(command, tool) {
  if (!/^(?:bash|powershell)$/i.test(tool)) return [];
  const seen = new Set(); const invocations = [];
  for (const candidate of taskloopCommandCandidates(command, tool)) {
...
function taskloopIdentityAssignments(command, tool) {
  const assignments = [];
  for (const candidate of taskloopCommandCandidates(command, tool)) {
    for (const segment of candidate.split(/[;&|\n\r]+/)) {
      const parsed = taskloopPrefix(segment.trim().replace(/^\(\s*/, ""), tool);
      if (parsed?.explicit !== null || parsed?.explicitActor !== null) assignments.push(parsed);
    }
  }
  return assignments;
}
function taskloopInvocation(command, tool) {
  const invocations = taskloopInvocations(command, tool);
  return invocations.length === 1 ? invocations[0] : null;
}
function taskloopFrictionCandidate(command, tool) {
  if (!/^(?:bash|powershell)$/i.test(tool)) return false;
  const invocations = taskloopInvocations(command, tool);
  return invocations.length > 1 || String(command).split(/[;&|\n\r]+/).some((segment) => taskloopInvocation(segment.trim().replace(/^\(\s*/, ""), tool));
}
function allowTaskloopCommand(sessionId, mapping, tool, agentId = null) {
  const commands = commandValues(mapping);
  if (commands.length !== 1) return 0;
  const assignments = taskloopIdentityAssignments(commands[0], tool);
  const taskloopShaped = taskloopFrictionCandidate(commands[0], tool) || assignments.some((item) => taskloopExecutable(item.invocation));
  ...
  if (taskloopShaped && assignments.some((item) => item.explicit !== null && item.explicit !== sessionId)) return deny("TASKLOOP_SESSION_ID conflicts with the Codex hook session_id");
  const invocations = taskloopInvocations(commands[0], tool);
  if (!invocations.length) return 0;
  if (!isSafeHookSessionId(sessionId)) return 0;
  if (invocations.some((invocation) => invocation.explicit !== null && invocation.explicit !== sessionId)) return deny("TASKLOOP_SESSION_ID conflicts with the Codex hook session_id");
...
function cmdLedger(values) {
  ...
  for (const record of authority.replay?.records ?? []) for (const event of record.events ?? []) appendUnanchoredUserClaims(userClaimRows, record, event, previousAssuranceByTask);
...
function hookPretool(invocation, repo) {
  const mapping = invocation.toolInput;
  const tool = invocation.toolName;
  const sessionId = invocation.sessionId;
  const actingSession = hookActor(invocation);
  const payload = { session_id: sessionId };
  const hookCommands = commandValues(mapping);
  if (hookCommands.some((command) => taskloopInvocation(command, tool))) {
    appendHookEvidence(repo, invocation, { kind: "actor_anchor", foreign: false, gate: invocation.mode, targets_parsed: [], during_task: null });
  } else if (hookCommands.some((command) => taskloopFrictionCandidate(command, tool))) {
  ...
  try { return withTaskLock(repo, () => {
    ...
    if (!current) {
      const noTaskShapes = commandValues(mapping).flatMap(commandShapes);
      const writeShaped = gitOps(mapping).length > 0 || looksLikeWrite(tool, mapping) || hasCommandWriteTarget(tool, mapping) || hasWriteCommandShape(noTaskShapes);
      const result = observeUntracked({ payload, sessionId, mode: invocation.mode, repo, writeShaped, writeTargets: writeFileTargets(tool, mapping), scriptPath: process.argv[1] ?? "taskloop.mjs" });
      ...
      return allowTaskloopCommand(sessionId, mapping, tool, invocation.agentId);
    }
    if (current.lifecycle.state === "terminal") return allowTaskloopCommand(sessionId, mapping, tool, invocation.agentId);
    const ops = gitOps(mapping);
    const shapes = commandValues(mapping).flatMap(commandShapes);
    const writeShaped = ops.length > 0 || looksLikeWrite(tool, mapping) || hasCommandWriteTarget(tool, mapping) || hasWriteCommandShape(shapes);
    if (writeShaped) appendHookEvidence(repo, invocation, {
      ...
      targets_parsed: writeFileTargets(tool, mapping).map((value) => repoRelative(repo, value)).filter(Boolean),
    ...
    const persistDeny = (reason) => {
      commitTaskCommand(repo, { type: "authorize-write", taskId: current.task_id, decision: "deny", reason, transcriptRange: range, actingSession, at, atEpochMs }, { actorKind: "hook", authority });
      return deny(reason);
    };
    if (current.lifecycle.state === "suspended") {
      ...
      if (range) commitTaskCommand(repo, { type: "tally-transcript", taskId: current.task_id, transcriptRange: range, actingSession, at, atEpochMs }, { actorKind: "hook", authority });
      return allowTaskloopCommand(sessionId, mapping, tool, invocation.agentId);
    }
    ...
    const commandMarkers = [...new Set(shapes.map((shape) => `<command:${shape}>`))];
    commitTaskCommand(repo, { type: "authorize-write", taskId: current.task_id, decision: "allow", reason: "", transcriptRange: range, files: [...(targets.length ? targets : ["<command>"]), ...commandMarkers], actingSession, at, atEpochMs }, { actorKind: "hook", authority });
    return allowTaskloopCommand(sessionId, mapping, tool, invocation.agentId);
  }); }
  catch (error) {
...
function dispatchHook(profile = "unknown", mode = "deny") {
  ...
  const mapping = invocation.toolInput;
  const writeShaped = gitOps(mapping).length > 0 || looksLikeWrite(invocation.toolName, mapping) || hasCommandWriteTarget(invocation.toolName, mapping) || hasWriteCommandShape(commandValues(mapping).flatMap(commandShapes));
  let result;
  try { if (event === "pre_tool_use") result = hookPretool(invocation, repo); else if (event === "stop") result = hookStop(repo, invocation); else return 0; }
  catch (err) {
    if (err?.hook_task_state_unavailable === true) {
      process.stderr.write(`${err.message}\n`);
      if (event === "pre_tool_use" && writeShaped) result = deny("supervisor unavailable; refusing a write whose artifact revision cannot be recorded");
      else if (event === "stop") result = holdDisposition("task_state_unavailable", "task state unavailable; refusing to adjudicate Stop");
    }
    else {
      process.stderr.write(`taskloop: supervisor error: ${err?.message ?? err}\n`);
      if (event === "pre_tool_use" && writeShaped) result = deny("supervisor unavailable; refusing a write whose artifact revision cannot be recorded");
      else if (event === "stop") result = holdDisposition("supervisor_unavailable", "supervisor unavailable; refusing to adjudicate Stop");
    }
  }
