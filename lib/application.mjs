...
import { changedSnapshotPaths, criterionDrift, criterionMessage, criterionMetadata, repoSnapshot, resolveCriterionFile, resolveSubject, runCriterionSource, validateRepoSnapshot } from "./criterion.mjs";
...
import { EXPLICIT_PROFILES, STOP_INLINE_CRITERION_SECONDS, STOP_RUNTIME_DEADLINE_SECONDS, buildHookRecipe, decodeHook, encodeHook, hostProfileCapability } from "./host-hooks.mjs";
...
function prepareTaskObservation(repo, options = {}, readiness = null) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    observationDeadlineEpochMs ??= Date.now() + candidate.task.criterion.timeout_seconds * 1000;
    ...
    if (bound.status !== "retry") return bound;
  }
  return {
    status: "hold",
...
function executePreparedObservation(repo, prepared, { intent, runtimeDeadlineSeconds = null, runtimeDeadlineEpochMs = null } = {}) {
  ...
  const deadlineEpochMs = runtimeDeadlineEpochMs ?? prepared.deadline_epoch_ms ?? startedAtEpochMs + deadlineSeconds * 1000;
...
function commitPreparedObservation(repo, prepared, executed, action, { recordSideEffect = true } = {}) {
  ...
  return withTaskLock(repo, () => {
    ...
    if (!authorityMatches) {
      ...
      return {
        ...
        evidence: sideEffect && recordSideEffect && sameMutableTask
          ? action({ authority, task: current, authorityChanged: true })
          : null,
      };
    }
    if (sideEffect) {
      ...
      return { status: "stale", evidence: recordSideEffect ? action({ authority, task: current, authorityChanged: false }) : null };
    }
    ...
    return { status: "committed", value: action({ authority, task: current }) };
  });
}
...
function cmdOpen(values) {
  ...
  const openDeadlineEpochMs = Date.now() + timeout * 1000;
  ...
  const committed = commitPreparedObservation(repo, prepared, executed, ({ authority }) => {
    const atEpochMs = Date.now(); const at = utcTimestamp(atEpochMs);
    observation.observed_at = at;
    return commitTaskCommand(repo, {
      type: "open", taskId, at, atEpochMs, actingSession: actingSessionId(),
