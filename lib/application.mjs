...
function historyRequirementFrom(values, current = null) {
  ...
  if (explicit === "artifact_only" && finiteWrites) throw new Error("finite write budget requires complete mutation history");
  if (explicit) return explicit;
  if (effectiveRisk === "critical" || finiteWrites) return "complete";
  return current?.history_requirement ?? "artifact_only";
}

function currentScopeViolations(task, checkpoint) {
  if (task?.runtime_contract !== 6) return [];
...
function commitTaskCommand(repo, command, { actorKind = "cli", commandId = null, openNewTask = false, onEventSeam = null, onSnapshotSeam = null, authority = null } = {}) {
  ...
  if (
    RUNTIME_CONTRACT >= 6 && !openNewTask && current?.runtime_contract !== 6 &&
    current?.lifecycle?.state !== "terminal" && command.type !== "abandon"
  ) {
    throw authorityError(
      "ACTIVE_CONTRACT_5_READ_ONLY",
      "active Contract 5 task is read-only under runtime 6; use the pinned Contract 5 runtime to finish it, or explicitly abandon it",
    );
  }
...
function bindTaskObservationSnapshot(repo, candidate, options, readiness, repositorySnapshot, deadlineEpochMs) {
  ...
  if (current.task.runtime_contract === 6) {
    ...
    if (checkpoint.checkpoint_id !== current.task.artifact_checkpoint.checkpoint_id) {
      ...
      const prewriteEnforcement = current.task.authority.prewrite_enforcement === "full" ? "partial" : current.task.authority.prewrite_enforcement;
...
function commitCriterionSideEffect(repo, {
  ...
  const trusted = task.runtime_contract === 6 && repositorySnapshot !== null;
  const checkpoint = trusted ? artifactCheckpointFromSnapshot(repositorySnapshot) : task.artifact_checkpoint;
  const delta = task.runtime_contract === 6 && trusted
  ...
  const prewriteEnforcement = !trusted ? "unknown"
    : task.authority.prewrite_enforcement === "full" ? "partial" : task.authority.prewrite_enforcement;
  return commitTaskCommand(repo, {
    ...
    ...(task.runtime_contract === 6 ? { reconciliation: {
...
function writeEvidenceProjection(task) {
  if (task?.runtime_contract !== 6) return null;
  return {
    history_requirement: task.history_requirement,
    write_count_basis: task.spent.write_count_basis,
    write_operations_authorized: task.authority.write_operations_authorized,
...
function writeBudgetCompliance(task) {
  ...
  if (task.authority.write_operations_authorized > task.budget.writes) return "exceeded";
  return task.authority.prewrite_enforcement === "full" && task.evidence.mutation_history_coverage === "full"
    ? "compliant"
    : "unknown";
}

function budgetProjection(task) {
  return task?.runtime_contract === 6
...
function cmdStatus(values) {
  ...
  return withTaskLock(repo, () => {
    ...
    process.stdout.write(JSON.stringify({
      ...
      spent: task.spent,
      artifact_revision: task.artifact_revision,
      ...(task.runtime_contract === 6 ? { write_evidence: writeEvidenceProjection(task) } : {}),
...
function cmdReport(values) {
  ...
  if (task.runtime_contract === 6) lines.push("", "## Write evidence", "", `- authorized operations: ${task.authority.write_operations_authorized} (${task.spent.write_count_basis})`, `- completion receipts: ${task.evidence.tool_completions_observed}`, `- artifact revisions: ${task.artifact_revision}`, `- artifact state coverage: ${task.evidence.artifact_state_coverage}`, `- mutation history coverage: ${task.evidence.mutation_history_coverage}`, `- prewrite enforcement: ${task.authority.prewrite_enforcement}`, `- history requirement: ${task.history_requirement}`);
  ...
  lines.push("", "## Budget", "", task.runtime_contract === 6
    ? `- rounds ${task.spent.rounds}/${task.budget.rounds}; authorization budget ${task.authority.write_operations_authorized}/${reportBudgetLimit(task.budget.writes)} (${writeBudgetCompliance(task)}); wall clock ${Math.round(projectedWallClockMs / 1000)}s/${reportBudgetLimit(task.budget.wall_clock_minutes, "m")}; output tokens estimate ${task.spent.output_tokens_estimate}/${reportBudgetLimit(task.budget.output_tokens)} (best effort)`
...
function terminalNotNeededContract6(repo, evidence) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const candidate = withTaskLock(repo, () => {
      const authority = loadV3Authority(repo, { recoverTail: true });
      const task = authority.projection;
      if (!task) return { status: "no_task" };
      if (task.runtime_contract !== 6) return { status: "legacy" };
    ...
    const committed = withTaskLock(repo, () => {
      ...
      const prewriteEnforcement = current.authority.prewrite_enforcement === "full" && delta.changed_paths.length
        ? "partial" : current.authority.prewrite_enforcement;
...
function terminalCommand(values, type) {
  ...
  if (type === "not-needed") {
    ...
    const contract6 = terminalNotNeededContract6(repo, values.evidence);
    if (contract6.status !== "legacy") {
      if (contract6.status === "terminal") {
        process.stdout.write(`workloop: terminal(${contract6.projection.lifecycle.outcome})\n`);
        return 0;
      }
      if (contract6.status === "no_task") return error("no task");
      if (contract6.status === "inactive") return error(`task is ${contract6.state}`);
      if (contract6.status === "held") return error(contract6.message);
...
function cmdLedger(values) {
  ...
  for (const event of authority.replay?.events ?? []) {
    ...
    if (event.kind === "task_opened") contractByTask.set(event.task_id, event.payload_version === 2 ? 6 : 5);
    ...
    if (event.kind === "artifact_reconciled") {
      ...
      if (event.payload.from_checkpoint !== event.payload.to_checkpoint) artifactChanges += 1;
  ...
  const terminalWriteSets = authorityFailure ? "unknown" : [...terminalByTask.entries()].map(([taskId, terminal]) => ({
    ...
    write_count_basis: contractByTask.get(taskId) === 6 ? "authorized" : "preauthorization_legacy",
  ...
  const payload = {
    ...
    metrics: {
      ...
      rounds, writes, write_count_basis: new Set(contractByTask.values()).size > 1 ? "mixed" : [...contractByTask.values()][0] === 6 ? "authorized" : "preauthorization_legacy", tool_completions: toolCompletions,
...
function hookPosttool(invocation, repo, callAnalysis) {
  ...
  if (!writeShapedCall(invocation.toolName, invocation.toolInput, callAnalysis)) return disposition;
  ...
  return withTaskLock(repo, () => {
    const authority = loadV3Authority(repo, { recoverTail: true });
    const current = authority.projection;
    if (!current || current.runtime_contract !== 6 || current.lifecycle.state === "terminal") return disposition;
    ...
    const authorized = current.operations[operationId]?.authorization !== null && current.operations[operationId]?.authorization !== undefined;
    const mutationHistory = !scanTrusted || !authorized ? "unknown"
    ...
    const prewriteEnforcement = !scanTrusted || !authorized ? "unknown"
      : !capability.exhaustive_surface && current.authority.prewrite_enforcement === "full" ? "partial"
        : current.authority.prewrite_enforcement;
    const targets = writeFileTargets(invocation.toolName, invocation.toolInput, callAnalysis)
      .map((value) => repoRelative(repo, value)).filter(Boolean);
    const atEpochMs = Date.now(); const at = utcTimestamp(atEpochMs);
    if (isForeignSession(current, invocation.sessionId)) {
      commitTaskCommand(repo, {
    ...
    commitTaskCommand(repo, {
      ...
      coverageChange: {
        ...
        reason: !scanTrusted ? "operation receipt observed but repository snapshot was unavailable"
          : authorized ? "operation completion receipt" : "orphan operation completion receipt",
      },
      actingSession: hookActor(invocation), at, atEpochMs,
    }, { actorKind: "hook", authority, commandId: invocation.commandId });
    return disposition;
...
function persistPostCommitFailure(repo, invocation, cause) {
  return withTaskLock(repo, () => {
    const authority = loadV3Authority(repo, { recoverTail: true });
    const current = authority.projection;
    if (!current || current.runtime_contract !== 6 || current.lifecycle.state === "terminal") return false;
    const atEpochMs = Date.now(); const at = utcTimestamp(atEpochMs);
    const operationId = invocation.commandId ?? null;
    const capability = hostProfileCapability(invocation.profile);
    const checkpointId = current.artifact_checkpoint.checkpoint_id;
    const diagnostic = String(cause?.code ?? cause?.message ?? cause).split("\n")[0].slice(0, 300) || "unknown";
    commitTaskCommand(repo, {
...
function hookStop(repo, invocation) {
  ...
  if (shouldRecordCensus) {
    ...
    appendHookEvidence(
      ...
      capability.stop_control === "release_only" ? { timeoutMs: 25 } : undefined,
    );
  }
  if (capability.stop_control === "release_only") return { event: "stop", action: "release" };
...
function dispatchHook(profile = "unknown", mode = "deny") {
  ...
  const writeShaped = writeShapedCall(invocation.toolName, mapping, callAnalysis);
  let result;
  try {
    ...
    if (new Set(["post_tool_use", "post_tool_use_failure"]).has(event)) {
      try {
        persistPostCommitFailure(repo, invocation, err);
        process.stderr.write(`workloop: completion receipt degraded and unknown coverage persisted: ${err?.message ?? err}\n`);
        return emitHookResult(invocation, { event, action: "record" });
      } catch (degradeError) {
        process.stderr.write(`workloop: completion receipt degradation could not be persisted: ${degradeError?.message ?? degradeError}; original error: ${err?.message ?? err}\n`);
        return 2;
      }
    }
...
function cmdHook(values) {
  ...
  return dispatchHook(values.profile, values.mode ?? "deny");
}
...
const COMMANDS = {
  help: () => {
    ...
    process.stdout.write(`note: claude is hard with a ${STOP_INLINE_CRITERION_SECONDS}s inline criterion budget; codex-safe and codex-cli-legacy are release-only\n`);
