...
const OPTIONS = {
  ...
  amend: { repo: OPTION, reason: OPTION, goal: OPTION, criterion: OPTION, "criterion-file": OPTION, "criterion-protocol": OPTION, "criterion-policy": OPTION, "criterion-authored-by": OPTION, "criterion-timeout-seconds": OPTION, "alignment-because": OPTION, "not-covered": { type: "string", multiple: true }, files: { type: "string", multiple: true }, "criterion-subject": { type: "string", multiple: true }, ...ASSURANCE_OPTIONS, rounds: OPTION, writes: OPTION, "wall-clock-minutes": OPTION, "token-budget": OPTION, "git-allowed": { type: "string", multiple: true }, "git-reason": OPTION, "destructive-allowed": BOOL, "destructive-scope": { type: "string", multiple: true }, "network-allowed": BOOL, "install-scripts-allowed": BOOL, "publish-allowed": BOOL, "granted-by": OPTION },
...
function commitTaskCommand(repo, command, { actorKind = "cli", commandId = null, openNewTask = false, onEventSeam = null, onSnapshotSeam = null, authority = null } = {}) {
  ...
  // Reduce before persisting. The reducer enforces invariants the event
  // builders do not (an open whose policy forbids its own observed verdict is
  // the one that bit us), and the log is append-only: a record the reducer
  // rejects would replay into the same throw on every later load, leaving the
  // repository permanently unreadable with no in-band recovery. Validating
  // first costs one extra reduction and keeps the rejection a plain command
  // error.
...
// attempt streak, and stuck detection. Same lock-held shape as the Stop path;
// the episode stays open because the session continues.
function cmdVerifyRecord(repo) {
  return withTaskLock(repo, () => {
    const task = readTask(repo, { recoverTail: true });
    if (!task) return error("no task");
    if (task.lifecycle.state !== "active") return error("verify --record requires an active task; resume first");
    ...
    if (observation.execution.execution_error === "criterion_side_effect") {
      const committed = commitTaskCommand(repo, { type: "criterion-side-effect", taskId: task.task_id, observation, actingSession: actingSessionId(), at, atEpochMs });
      process.stdout.write(JSON.stringify({ ...contractDescriptor(), observation, recorded: committed.committed, status: committed.result.status }) + "\n");
      return 2;
    }
...
function cmdAmend(values) {
  ...
  return withTaskLock(repo, () => {
    ...
    // The timeout is part of the criterion definition, so amending it is a
    // criterion change. Without this the only way to correct a timeout that
    // makes every observation indeterminate — a suite that outgrew it, say —
    // was to abandon the task and open a new one.
    const changesCriterion = values.criterion !== undefined || values["criterion-file"] !== undefined || values["criterion-protocol"] !== undefined || values["criterion-subject"] !== undefined || values["criterion-authored-by"] !== undefined || values["criterion-timeout-seconds"] !== undefined;
    if (changesCriterion) {
      ...
      const timeoutSeconds = values["criterion-timeout-seconds"] !== undefined
        ? parseInteger(values["criterion-timeout-seconds"], "criterion-timeout-seconds", { minimum: 1 })
        : task.criterion.timeout_seconds;
      event.criterion = criterionMetadata({ source, protocol, timeoutSeconds, subjects, authoredBy, repo });
...
// Failures the loader attributes to the snapshot: the event authority is fine
// (or absent) and the snapshot is the artifact to retire. Everything else —
// corrupt records, a tail that cannot replay, a reducer that rejects an event —
// belongs to the event log.
const SNAPSHOT_OWNED_AUTHORITY_FAILURES = new Set(["LEGACY_STATE_UNSUPPORTED", "ORPHAN_V3_SNAPSHOT", "MIXED_OR_INVALID_AUTHORITY"]);
// Naming states, not damage: both artifacts may be perfectly readable and the
// sanctioned repair is migrate-artifact-names. Archiving here would retire a
// healthy — possibly the authoritative — artifact and still not clear the
// state, so this verb refuses and says which one does the job.
const ARTIFACT_NAME_FAILURES = new Set(["ARTIFACT_NAME_CONFLICT", "ARTIFACT_NAME_MIGRATION_REQUIRED"]);

function incompatibleArtifactPath(repo) {
  const snapshot = taskPath(repo);
  const eventStore = eventStorePath(repo);
  let code = null;
  try { loadV3Authority(repo, { allowEmpty: false, recoverTail: true }); }
  catch (cause) { code = cause?.code ?? "EVENT_STORE_AUDIT_FAILED"; }
  if (code && ARTIFACT_NAME_FAILURES.has(code)) return { refusal: "artifact names disagree; run migrate-artifact-names --granted-by user instead of archiving a healthy artifact" };
  // Nothing failed to load, so there is no incompatible state to retire.
  if (!code) return { refusal: "task state and event authority both load; there is no incompatible state to archive" };
  if (!SNAPSHOT_OWNED_AUTHORITY_FAILURES.has(code) && fs.existsSync(eventStore)) return { source: eventStore };
  if (fs.existsSync(snapshot)) return { source: snapshot };
  return fs.existsSync(eventStore) ? { source: eventStore } : { refusal: "no incompatible task state or event record to archive" };
}

function cmdArchive(values) {
  const repo = repoOf(values);
  try { return withTaskLock(repo, () => {
    // Archive the artifact that actually fails to load, which existence alone
    // cannot tell you: the snapshot is rewritten on every commit, so it is
    // usually present and healthy exactly when the event log is the damaged
    // one. Preferring it by presence archives the good artifact, leaves the
    // damage in place, and then throws recovering from it.
    const { source, refusal } = incompatibleArtifactPath(repo);
    if (refusal) return error(refusal);
    const receipt = archiveIncompatibleState(repo, { reason: values.reason, grantedBy: values["granted-by"], at: now(), source });
