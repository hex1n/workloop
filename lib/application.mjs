  ...
  return withTaskLock(repo, () => {
    ...
    const bindingNextAction = task.lifecycle.state === "terminal" ? null
      : task.lifecycle.state === "suspended" ? "resume --reason R in the continuing host session"
        : owner && cliIdentity !== owner ? "taskloop join --reason R to continue this task; use a separate worktree for parallel work"
          : null;
...
function terminalCommand(values, type) {
  ...
  return withTaskLock(repo, () => {
    ...
    process.stdout.write(`taskloop: terminal(${result.projection.lifecycle.outcome})\n`);
    return 0;
  });
}
