...
const OPTIONS = {
  ...
  audit: { repo: OPTION }, "sync-outcomes": { repo: OPTION }, "audit-outcomes": {}, info: {}, hooks: { profile: OPTION }, hook: { profile: OPTION }, help: {},
};
...
function allowTaskloopCommand(sessionId, mapping, tool) {
...
function adjudicateClosure(repo, { explicit, stop, sessionId, transcriptPath }, readiness) {
  return withTaskLock(repo, () => {
    const task = readTask(repo, { recoverTail: true });
    ...
    if (committed.result.status === "suspended") {
      ...
      const message = `criterion unsatisfied; task suspended(${committed.result.reason}): ${judgment.failure}`;
...
function closeAttempt(repo, { explicit, stop = false, sessionId = null, transcriptPath = null }) {
  ...
  try {
    const result = adjudicateClosure(repo, { explicit, stop, sessionId, transcriptPath }, readiness);
...
function hookPretool(invocation, repo) {
  ...
  try { return withTaskLock(repo, () => {
    ...
    if (!current) {
      ...
      return allowTaskloopCommand(sessionId, mapping, tool);
    }
    if (current.lifecycle.state === "terminal") return allowTaskloopCommand(sessionId, mapping, tool);
...
function hookStop(repo, invocation) {
  return closeAttempt(repo, { explicit: false, stop: true, sessionId: invocation.sessionId, transcriptPath: invocation.transcriptPath });
}
...
function dispatchHook(profile = "unknown") {
  const payload = loadPayload(); const invocation = decodeHook({ profile, payload }); const event = invocation.event; const repo = path.resolve(invocation.repo);
  const mapping = invocation.toolInput;
  const writeShaped = gitOps(mapping).length > 0 || looksLikeWrite(invocation.toolName, mapping);
  let result;
