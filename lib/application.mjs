...
import { mutateCurrentGitTask, openCurrentGitTask, queryCurrentGit, recordCurrentGitHook } from "./git-authority-provider.mjs";
...
const OPTIONS = {
  ...
  "current-open": { target: OPTION, placement: OPTION, "worktree-path": OPTION, branch: OPTION, base: OPTION, goal: OPTION, "write-path": { type: "string", multiple: true }, "write-root": { type: "string", multiple: true }, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-status": { target: OPTION, "task-id": OPTION },
  "current-audit": { target: OPTION, "task-id": OPTION },
  "current-ledger": { target: OPTION },
  "current-tasks": { target: OPTION },
  "current-join": { target: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-suspend": { target: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-resume": { target: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-abandon": { target: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
...
function cmdCurrentOpen(values) {
  return printCurrent(openCurrentGitTask({
    target: values.target, placement: values.placement, worktreePath: values["worktree-path"], branch: values.branch, base: values.base,
...
function cmdCurrentQuery(values, kind) {
  ...
  const sessionId = hostSessionId();
  return printCurrent(queryCurrentGit(values.target, kind, CURRENT_AUTHORITY_RUNTIME, { taskId: values["task-id"] ?? null, sessionId: sessionId === "cli" ? null : sessionId }));
}

function cmdCurrentTaskMutation(values, action) {
  return printCurrent(mutateCurrentGitTask({
    action, target: values.target, taskId: values["task-id"], commandId: values["command-id"],
    sessionId: hostSessionId(), grantedBy: values["granted-by"], reason: values.reason,
  }, CURRENT_AUTHORITY_RUNTIME));
}
...
function cmdCurrentHook(values) {
  ...
  for (const target of targets) {
    try { recordCurrentGitHook({ target, invocation }, CURRENT_AUTHORITY_RUNTIME); }
