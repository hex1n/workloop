...
import { STATE_DIR, TASK_FILE, DEFAULT_ROUNDS, CRITERION_TIMEOUT_SECONDS, VALID_SUSPEND_OUTCOMES, REVIEW_LEVELS, isPlainObject, fnv1aHex, outputTail, localStamp, repoRelative } from "./prims.mjs";
...
function cmdOpen(values) {
  ...
  try {
    return withTaskLock(repo, () => {
      ...
      if (current) {
        try {
          archiveTask(repo, current, localStamp());
        } catch (err) {
          process.stderr.write(
...
function cmdSuspend(values) {
  ...
  task = transition(task, { type: "suspend", at: localStamp(), outcome, source: "user", judgment }).task;
  saveTask(repo, task);
...
function cmdResume(values) {
  ...
  task = transition(task, { type: "resume", at: localStamp(), reason }).task;
  saveTask(repo, task);
...
function cmdAbandon(values) {
  ...
  task = transition(task, { type: "terminal", at: localStamp(), state: "abandoned", episodeOutcome: "detached", reason }).task;
  saveTask(repo, task);
...
function cmdNotNeeded(values) {
  ...
  task = transition(task, { type: "terminal", at: localStamp(), state: "not_needed", episodeOutcome: "green", reason: evidence }).task;
  saveTask(repo, task);
...
function cmdReview(values) {
  ...
  const record = { at: localStamp(), level, criterion_hash: task.criterion_hash };
...
function cmdAmend(values) {
  ...
  const amendment = { at: localStamp(), reason };
...
function hookPretoolLocked(payload, repo, currentTask) {
  ...
  const episode = transition(currentTask, { type: "begin-episode", at: localStamp(), atMs: Date.now(), session: payload.session_id });
  ...
  task = transition(task, { type: "record-write", at: localStamp(), files: rels }).task;
  saveTask(repo, task);
  return 0;
}

function suspendByMachine(repo, currentTask, outcome, note) {
  const task = transition(currentTask, { type: "machine-suspend", at: localStamp(), outcome, note }).task;
  saveTask(repo, task);
...
function hookStopLocked(payload, repo, currentTask) {
  ...
  if (verdict.verdict === "pass" && task.keep_green) {
    ...
    task = transition(task, { type: "keep-green", at: localStamp() }).task;
    saveTask(repo, task);
    return 0;
  }
  ...
  const failure = transition(task, { type: "criterion-failure", at: localStamp(), verdict, signature });
  task = failure.task;
