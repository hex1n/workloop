...
const OPTIONS = {
  ...
  status: { repo: OPTION }, verify: { repo: OPTION }, achieve: { repo: OPTION },
...
function closeAttempt(repo, { explicit, stop = false, sessionId = null, transcriptPath = null }) {
  return withTaskLock(repo, () => {
    ...
    if (suspensionReason) {
      ...
      next = transition(next, { type: "suspend", reason: suspensionReason, judgment, source: stop ? "stop" : "cli", actingSession, at }).task;
...
function main() {
  ...
  try {
    ...
    if (verb === "open") return cmdOpen(values); if (verb === "status") return cmdStatus(values); if (verb === "verify") return cmdVerify(values); if (verb === "achieve") return closeAttempt(repoOf(values), { explicit: true });
