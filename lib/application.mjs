...
import { mutateCurrentGitTask, openCurrentGitTask, queryCurrentGit, recordCurrentGitHook, recoverCurrentGitAttachment, forkCurrentGitIdentity } from "./git-authority-provider.mjs";
...
function cmdCurrentHook(values) {
  ...
  for (const target of targets) {
    try { if (currentFilesystemProvider(target)) recordCurrentFilesystemHook({ target, invocation }, CURRENT_AUTHORITY_RUNTIME); else recordCurrentGitHook({ target, invocation }, CURRENT_AUTHORITY_RUNTIME); }
