...
import { assertTaskSchema, closureProjection, constructAssurance, constructPolicy, criterionDefinitionHash, createTask, machineRiskFloor, policyName, projectAssurance, projectProofAssurance, projectReviewRequirement, transition, validateAssurance } from "./task-engine.mjs";
...
function cmdReport(values) {
  ...
  lines.push("", "## Budget", "", `- rounds ${task.spent.rounds}/${task.budget.rounds}; writes ${task.spent.writes}${task.budget.writes ? `/${task.budget.writes}` : ""}; wall clock ${Math.round(task.spent.wall_clock_ms / 1000)}s`);
...
function failureSuspension(task, observation) {
  ...
  if (task.spent.rounds >= task.budget.rounds) return { reason: "out_of_budget", failure: `round budget exhausted (${task.spent.rounds}/${task.budget.rounds})` };
...
function closeAttempt(repo, { explicit, stop = false, sessionId = null, transcriptPath = null }) {
  return withTaskLock(repo, () => {
    ...
    const suspension = failureSuspension(next, observation);
    if (suspension) {
      const judgment = {
        remaining: `criterion must become satisfied: ${next.criterion.source.value}`,
        failure: suspension.failure,
        next_action: suspension.reason === "out_of_budget" ? "amend --rounds with a reason, then resume" : "change the approach or inputs, then resume",
      };
...
function cmdResume(values) {
  ...
  return withTaskLock(repo, () => {
    ...
    try { next = transition(task, { type: "resume", reason: values.reason, episode, actingSession: episode.host_session_id, at }).task; } catch (err) { return error(err.message); }
...
function hookPretool(payload, repo, task) {
  ...
  return withTaskLock(repo, () => {
    ...
    recordOwnerHookContact(next, sessionId, now());
    saveTask(repo, next);
    return allowTaskloopCommand(payload, mapping, tool);
  });
}
