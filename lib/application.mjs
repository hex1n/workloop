...
import { mutateCurrentGitTask, openCurrentGitTask, queryCurrentGit, recordCurrentGitHook, recoverCurrentGitAttachment, forkCurrentGitIdentity, stageCurrentGitTask, commitCurrentGitTask, prepareCurrentGitCertification, certifyCurrentGitTask } from "./git-authority-provider.mjs";
import { abandonStagedFilesystemAuthority, forkCurrentFilesystemIdentity, mutateCurrentFilesystemTask, openCurrentFilesystemTask, queryCurrentFilesystem, queryCurrentFilesystemAuthority, recordCurrentFilesystemHook, recoverCurrentFilesystemAttachment, resolveFilesystemAuthorityTarget, prepareCurrentFilesystemCertification, certifyCurrentFilesystemTask } from "./filesystem-authority-provider.mjs";
...
const OPTIONS = {
  ...
  "current-open": { target: OPTION, "filesystem-root": OPTION, placement: OPTION, "worktree-path": OPTION, branch: OPTION, base: OPTION, goal: OPTION, "write-path": { type: "string", multiple: true }, "write-root": { type: "string", multiple: true }, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-stage": { target: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-certify": { target: OPTION, authority: OPTION, "task-id": OPTION, "criterion-file": OPTION, "criterion-timeout-seconds": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-commit": { target: OPTION, "task-id": OPTION, message: OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-status": { target: OPTION, authority: OPTION, "task-id": OPTION },
  "current-audit": { target: OPTION, authority: OPTION, "task-id": OPTION },
  "current-ledger": { target: OPTION, authority: OPTION },
  "current-tasks": { target: OPTION, authority: OPTION },
  "current-join": { target: OPTION, authority: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-suspend": { target: OPTION, authority: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-resume": { target: OPTION, authority: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-abandon": { target: OPTION, authority: OPTION, "task-id": OPTION, "command-id": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-recover-attachment": { target: OPTION, authority: OPTION, attachment: OPTION, "command-id": OPTION, "expect-epoch": OPTION, "expect-locator-digest": OPTION, "expect-pending-digest": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-cleanup-staged-locator": { target: OPTION, authority: OPTION, attachment: OPTION, "command-id": OPTION, "expect-locator-digest": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-reattach": { target: OPTION, authority: OPTION, attachment: OPTION, "command-id": OPTION, "expect-epoch": OPTION, "expect-locator-digest": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-abandon-staged-authority": { authority: OPTION, "command-id": OPTION, "expect-genesis-digest": OPTION, reason: OPTION, "granted-by": OPTION },
  "current-fork-identity": { target: OPTION, attachment: OPTION, "command-id": OPTION, "expect-epoch": OPTION, "expect-locator-digest": OPTION, reason: OPTION, "granted-by": OPTION },
...
function publishCurrentOutcome(value, { silent = false } = {}) {
  if (!value?.authority_id || !new Set(["git_common", "filesystem_detached"]).has(value.provider)) return value;
  if (!Array.isArray(value.repository_tasks) && !Array.isArray(value.filesystem_tasks)) return value;
  try {
    const published = publishAuthorityOutcome(value, CURRENT_AUTHORITY_RUNTIME);
    return { ...value, outcome_path: published.outcome_path, outcome_cursor_path: published.cursor_path, outcome_source_sequence: published.source_sequence };
  } catch (cause) {
    if (silent) return value;
    return { ...value, warnings: [...(value.warnings ?? []), `outcome projection deferred: ${cause.message}`] };
  }
}

function printCurrent(value) { process.stdout.write(`${JSON.stringify(publishCurrentOutcome(value), null, 2)}\n`); return 0; }

function currentFilesystemProvider(target) {
  try { resolveFilesystemAuthorityTarget(target); return true; }
  catch (cause) { if (cause?.code === "FILESYSTEM_AUTHORITY_REQUIRED") return false; throw cause; }
}

function cmdCurrentOpen(values) {
  const input = {
    target: values.target, filesystemRoot: values["filesystem-root"], placement: values.placement, worktreePath: values["worktree-path"], branch: values.branch, base: values.base,
    goal: values.goal, writePaths: values["write-path"], writeRoots: values["write-root"], commandId: values["command-id"],
    sessionId: hostSessionId(), grantedBy: values["granted-by"], reason: values.reason,
  };
  return printCurrent(values["filesystem-root"] ? openCurrentFilesystemTask(input, CURRENT_AUTHORITY_RUNTIME) : openCurrentGitTask(input, CURRENT_AUTHORITY_RUNTIME));
}

function cmdCurrentGitReceipt(values, action) {
  const target = normalizedIdentity(values.target);
  if (!target) return error(action + " requires --target");
  if (currentFilesystemProvider(target)) return error(action + " is unavailable for detached filesystem authorities");
  const input = { target, taskId: values["task-id"], message: values.message, commandId: values["command-id"], sessionId: hostSessionId(), grantedBy: values["granted-by"], reason: values.reason };
  return printCurrent(action === "stage" ? stageCurrentGitTask(input, CURRENT_AUTHORITY_RUNTIME) : commitCurrentGitTask(input, CURRENT_AUTHORITY_RUNTIME));
}

function cmdCurrentCertify(values) {
  const target = normalizedIdentity(values.target); const authorityId = normalizedIdentity(values.authority);
  if (!target && !authorityId) return error("current-certify requires --target or --authority");
  if (target && authorityId) return error("current-certify accepts exactly one of --target or --authority");
  const input = { target, authorityId, taskId: values["task-id"], commandId: values["command-id"], sessionId: hostSessionId(), grantedBy: values["granted-by"], reason: values.reason };
  const filesystem = authorityId || currentFilesystemProvider(target);
  const prepared = filesystem ? prepareCurrentFilesystemCertification(input, CURRENT_AUTHORITY_RUNTIME) : prepareCurrentGitCertification(input, CURRENT_AUTHORITY_RUNTIME);
  const source = { kind: "file", value: resolveCriterionFile(prepared.criterion_root, values["criterion-file"]) };
  const observation = runCriterionSource(source, prepared.criterion_root, Number(values["criterion-timeout-seconds"] ?? 120), "tri-state");
  if (observation.verdict !== "satisfied") return error("current-certify criterion " + observation.verdict + ": " + (observation.execution.output_tail ?? "no criterion receipt"));
  const digest = sha256Hex(JSON.stringify({ verdict: observation.verdict, execution: observation.execution, changed_paths: observation.changed_paths }));
  return printCurrent(filesystem ? certifyCurrentFilesystemTask(input, prepared, digest, CURRENT_AUTHORITY_RUNTIME) : certifyCurrentGitTask(input, prepared, digest, CURRENT_AUTHORITY_RUNTIME));
}

function cmdCurrentQuery(values, kind) {
  const target = normalizedIdentity(values.target); const authorityId = normalizedIdentity(values.authority);
  if (!target && !authorityId) return error(kind + " requires --target or --authority");
  if (target && authorityId) return error(kind + " accepts exactly one of --target or --authority");
  const sessionId = hostSessionId();
  const selection = { taskId: values["task-id"] ?? null, sessionId: sessionId === "cli" ? null : sessionId };
  if (authorityId) return printCurrent(queryCurrentFilesystemAuthority(authorityId, kind, CURRENT_AUTHORITY_RUNTIME, selection));
  return printCurrent(currentFilesystemProvider(target) ? queryCurrentFilesystem(target, kind, CURRENT_AUTHORITY_RUNTIME, selection) : queryCurrentGit(target, kind, CURRENT_AUTHORITY_RUNTIME, selection));
}

function cmdCurrentTaskMutation(values, action) {
  const target = normalizedIdentity(values.target); const authorityId = normalizedIdentity(values.authority);
  if (!target && !authorityId) return error(action + " requires --target or --authority");
  if (target && authorityId) return error(action + " accepts exactly one of --target or --authority");
  const input = { action, target, authorityId, taskId: values["task-id"], commandId: values["command-id"], sessionId: hostSessionId(), grantedBy: values["granted-by"], reason: values.reason };
  if (authorityId) return printCurrent(mutateCurrentFilesystemTask(input, CURRENT_AUTHORITY_RUNTIME));
  return printCurrent(currentFilesystemProvider(target) ? mutateCurrentFilesystemTask(input, CURRENT_AUTHORITY_RUNTIME) : mutateCurrentGitTask(input, CURRENT_AUTHORITY_RUNTIME));
}

function cmdCurrentRecovery(values, action) {
  const input = { action, target: values.target, authorityId: values.authority, attachmentId: values.attachment, commandId: values["command-id"], expectedEpoch: values["expect-epoch"], expectedLocatorDigest: values["expect-locator-digest"], expectedPendingDigest: values["expect-pending-digest"], grantedBy: values["granted-by"], reason: values.reason };
  return printCurrent(input.authorityId || currentFilesystemProvider(input.target) ? recoverCurrentFilesystemAttachment(input, CURRENT_AUTHORITY_RUNTIME) : recoverCurrentGitAttachment(input, CURRENT_AUTHORITY_RUNTIME));
}

function cmdCurrentForkIdentity(values) {
  const input = { target: values.target, attachmentId: values.attachment, commandId: values["command-id"], expectedEpoch: values["expect-epoch"], expectedLocatorDigest: values["expect-locator-digest"], grantedBy: values["granted-by"], reason: values.reason };
  return printCurrent(currentFilesystemProvider(input.target) ? forkCurrentFilesystemIdentity(input, CURRENT_AUTHORITY_RUNTIME) : forkCurrentGitIdentity(input, CURRENT_AUTHORITY_RUNTIME));
}

function cmdCurrentAbandonStagedAuthority(values) {
  return printCurrent(abandonStagedFilesystemAuthority({ authorityId: values.authority, commandId: values["command-id"], expectedGenesisDigest: values["expect-genesis-digest"], grantedBy: values["granted-by"], reason: values.reason }, CURRENT_AUTHORITY_RUNTIME));
}
...
function cmdCurrentHook(values) {
  ...
  if (!targets.length) failures.push("TARGET_ROUTING_UNAVAILABLE: current authority Hook requires at least one canonical target");
  for (const target of targets) {
    try {
      const receipt = currentFilesystemProvider(target)
        ? recordCurrentFilesystemHook({ target, invocation }, CURRENT_AUTHORITY_RUNTIME)
        : recordCurrentGitHook({ target, invocation }, CURRENT_AUTHORITY_RUNTIME);
      publishCurrentOutcome(receipt, { silent: true });
    }
    catch (cause) { failures.push(boundedHookDiagnostic(cause)); }
  }
  if (failures.length) {
...
const COMMANDS = {
  ...
  "current-stage": (values) => cmdCurrentGitReceipt(values, "stage"),
  "current-certify": cmdCurrentCertify,
  "current-commit": (values) => cmdCurrentGitReceipt(values, "commit"),
  ...
  "current-join": (values) => cmdCurrentTaskMutation(values, "join"),
  "current-suspend": (values) => cmdCurrentTaskMutation(values, "suspend"),
  "current-resume": (values) => cmdCurrentTaskMutation(values, "resume"),
  "current-abandon": (values) => cmdCurrentTaskMutation(values, "abandon"),
  "current-recover-attachment": (values) => cmdCurrentRecovery(values, "recover"),
  "current-cleanup-staged-locator": (values) => cmdCurrentRecovery(values, "cleanup"),
  "current-reattach": (values) => cmdCurrentRecovery(values, "reattach"),
  "current-abandon-staged-authority": cmdCurrentAbandonStagedAuthority,
  "current-fork-identity": cmdCurrentForkIdentity,
