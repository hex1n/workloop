...
import { runCriterion, runCriterionFile, criterionSensorMetadata, resolveCriterionFile, criterionFileSensorMetadata, runTaskCriterion, warnCriterionSensor, criterionInputDrift } from "./criterion.mjs";
import { commandValues, gitOps, commandSafetyFailure, looksLikeWrite, writeFileTargets, insideEnvelope, joinedFileOffender, joinedFilesMessage, envelopeDirty, warnZeroMatchEnvelope } from "./supervision.mjs";
import { activeEpisode, episodeOwnership, spentTokens, touchedSummary, resumeBanner, createTask, transition } from "./task-engine.mjs";
import { taskPath, loadTask, saveTask, archiveTask } from "./task-store.mjs";
...
function collectGrants({ grantedBy, flags = null, gitOps = [], gitReason = null, files = [], fileReason = null, sink = process.stderr }) {
  const grants = [];
  const at = utcNow();
...
function cmdOpen(values) {
  ...
  // Validate the replacement before moving the current task. A refused open
  // must be side-effect-free, otherwise retries archive the same live task
  // repeatedly and make history look like real task churn.
  if (existing) {
    try {
      archiveTask(repo, existing, utcNow());
    } catch (err) {
      process.stderr.write(
...
function warnInputDrift(drift, sink = process.stderr) {
  if (drift.length) {
    sink.write(
...
function adjudicateGreen(repo, currentTask, provisional = false) {
  const drift = criterionInputDrift(currentTask, repo);
  ...
  if (result.meta.outcome === "drift") warnInputDrift(drift);
  saveTask(repo, task);
...
function cmdAmend(values) {
  ...
  if (
    ...
    return cliError(
      "amend requires --goal, --criterion, --criterion-file, --criterion-protocol, --files, --rounds, --criterion-timeout-seconds, and/or --git-allowed",
    );
  }
  const grantedBy = String(values["granted-by"] ?? "self").trim() || "self";
  if (!GRANT_PROVENANCES.has(grantedBy)) {
...
function hookPretool(payload, repo, currentTask) {
...
function hookStop(payload, repo, currentTask) {
  const ownership = episodeOwnership(currentTask, payload.session_id, Date.now(), resolveLeaseTtlMs());
  if (ownership.role === "bystander") {
...
function cmdHelp() {
  process.stdout.write(
    "taskloop.mjs — task-first loop supervisor (clean-room v2)\n\n" +
      ...
      '  amend    --goal/--criterion/--criterion-file/--criterion-protocol/--files/--rounds/--criterion-timeout-seconds --reason "<why>"\n' +
...
function main() {
  ...
  try {
    const suspended = suspendedHookOutcome(event, payload, repo, task);
