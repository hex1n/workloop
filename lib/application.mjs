...
function loadV3Authority(repo, { allowEmpty = true, recoverTail = false } = {}) {
  ...
  const recovered = recoverV3TaskSnapshot(repo, { recoverTail });
...
function commitTaskCommand(repo, command, { actorKind = "cli", commandId = null, openNewTask = false, onEventSeam = null, onSnapshotSeam = null } = {}) {
  ...
  const current = openNewTask ? null : authority.projection;
  const outcome = decide(current, command);
  if (!outcome.events.length) return { ...outcome, projection: authority.projection, committed: false, warning: null };
  ...
  try { syncOutcomeRecords({ repoIdentity: repoIdentity(repo), records: [record] }); }
...
function hookPretool(payload, repo, task) {
  ...
  return withTaskLock(repo, () => {
    ...
    const persistDeny = (reason) => {
      commitTaskCommand(repo, { type: "authorize-write", taskId: current.task_id, decision: "deny", reason, transcriptRange: range, actingSession: sessionId, at, atEpochMs }, { actorKind: "hook" });
      return deny(reason);
    };
    if (current.lifecycle.state === "suspended") {
      ...
      if (range) commitTaskCommand(repo, { type: "tally-transcript", taskId: current.task_id, transcriptRange: range, actingSession: sessionId, at, atEpochMs }, { actorKind: "hook" });
      return allowTaskloopCommand(payload, mapping, tool);
    }
    ...
    commitTaskCommand(repo, { type: "authorize-write", taskId: current.task_id, decision: "allow", reason: "", transcriptRange: range, files: targets.length ? targets : ["<command>"], actingSession: sessionId, at, atEpochMs }, { actorKind: "hook" });
    return allowTaskloopCommand(payload, mapping, tool);
  });
}

function hookStop(repo, _task, payload) {
  const sessionId = payloadSessionId(payload);
...
function dispatchHook() {
  ...
  let task;
  try { task = withTaskLock(repo, () => readTask(repo, { recoverTail: true })); }
  catch (err) {
