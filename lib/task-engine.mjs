...
const V3_DECIDERS = {
  ...
  "complete-tool"(state, command) {
    ...
    if (taskRuntimeContract(state) !== 6) throw new Error("complete-tool requires a Contract 6 task");
    const values = {
      operation_id: command.operationId,
      tool_family: command.toolFamily,
  ...
  "reconcile-artifacts"(state, command) {
    ...
    return { events: [v3Event(command, "artifact_reconciled", values, state.task_id)], result: { status: "reconciled" } };
  },
...
const V3_REDUCERS = {
  ...
  tool_completed(state, event) {
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state !== "active") throw new Error("tool_completed requires an active Contract 6 task");
    const payload = event.payload;
