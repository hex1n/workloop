...
function currentClaimPath(value, kind) {
  ...
  if (parts.some((part) => !part || part === "." || part === "..") || new Set([".git", ".workloop"]).has(parts[0])) return false;
...
function assertCurrentAuthorityProjection(value) {
  currentAuthorityRequire(isPlainObject(value) && hasExactKeys(value, ["authority_state_schema_version", "authority_id", "provider", "authority_sequence", "command_ids", "attachments", "task_intents", "tasks"]), "projection fields");
  ...
  currentAuthorityRequire(Array.isArray(value.attachments) && Array.isArray(value.task_intents) && Array.isArray(value.tasks), "projection collections");
  ...
  for (const attachment of value.attachments) {
    currentAuthorityRequire(hasExactKeys(attachment, ["attachment_id", "stage_command_id", "lifecycle", "claim_token_digest", "claim_epoch", "anchor_id", "staged_locator_digest", "claimed_locator_digest", "stage_intent_digest", "staged_receipt_digest", "pending_record_digest", "final_record_digest", "worktree_root", "git_dir", "granted_by", "reason"]), "attachment fields");
    currentAuthorityRequire(isUuidV4(attachment.attachment_id) && v3NonEmpty(attachment.stage_command_id) && attachment.claim_epoch === 1 && v3NonEmpty(attachment.anchor_id), "attachment identity");
    ...
    currentAuthorityRequire(v3NonEmpty(attachment.worktree_root) && v3NonEmpty(attachment.git_dir), "attachment placement");
  ...
  for (const intent of value.task_intents) {
    currentAuthorityRequire(hasExactKeys(intent, ["task_id", "attachment_id", "command_id", "goal", "write_claims", "placement", "coordinator_session_id", "participant_session_ids", "status", "intent_record_digest"]), "task intent fields");
    ...
    currentAuthorityRequire(intent.placement === "partitioned" && v3NonEmpty(intent.coordinator_session_id) && isSha256Digest(intent.intent_record_digest), "task intent attachment");
  ...
  for (const task of value.tasks) {
    currentAuthorityRequire(hasExactKeys(task, ["task_id", "attachment_id", "goal", "write_claims", "placement", "coordinator_session_id", "participant_session_ids", "lifecycle", "attachment_final_digest", "operation_intents", "tool_completions"]), "task fields");
    currentAuthorityRequire(isUuidV4(task.task_id) && isUuidV4(task.attachment_id) && v3NonEmpty(task.goal), "task identity");
    currentAuthorityRequire(task.placement === "partitioned" && v3NonEmpty(task.coordinator_session_id) && isSha256Digest(task.attachment_final_digest), "task attachment");
    ...
    else if (task.lifecycle.state === "terminal") currentAuthorityRequire(hasExactKeys(task.lifecycle, ["state", "outcome", "reason"]) && task.lifecycle.outcome === "abandoned" && v3NonEmpty(task.lifecycle.reason), "terminal task lifecycle");
    ...
    currentAuthorityRequire(owningAttachment !== null && owningAttachment.lifecycle === "claimed" && task.attachment_final_digest === owningAttachment.final_record_digest, "task attachment reference");
    const openingIntent = value.task_intents.find((item) => item.task_id === task.task_id) ?? null;
    currentAuthorityRequire(openingIntent !== null && openingIntent.status === "opened" && openingIntent.attachment_id === task.attachment_id && openingIntent.goal === task.goal && canonicalJson(openingIntent.write_claims) === canonicalJson(task.write_claims) && openingIntent.coordinator_session_id === task.coordinator_session_id, "task open intent reference");
...
function evolveCurrentAuthority(state, record) {
  ...
  if (record.kind === "authority_genesis") {
    ...
    return assertCurrentAuthorityProjection({ authority_state_schema_version: 1, authority_id: payload.authority_id, provider: payload.provider, authority_sequence: 1, command_ids: [record.command_id], attachments: [], task_intents: [], tasks: [] });
  }
  ...
  if (record.kind === "attachment_stage_intent") {
    currentAuthorityRequire(attachment === null && next.attachments.every((item) => item.lifecycle === "claimed"), "stage exclusivity");
    currentAuthorityRequire(!next.attachments.some((item) => item.anchor_id === payload.anchor_id), "attachment anchor uniqueness");
    currentAuthorityRequire(isUuidV4(payload.attachment_id) && isUuidV4(payload.claim_token) && payload.claim_epoch === 1 && v3NonEmpty(payload.anchor_id), "stage identities");
    ...
    currentAuthorityRequire(isUuidV4(payload.task_id) && v3NonEmpty(payload.goal) && payload.placement === "partitioned", "task open intent input");
    ...
    next.tasks.push({ task_id: taskIntent.task_id, attachment_id: taskIntent.attachment_id, goal: taskIntent.goal, write_claims: clone(taskIntent.write_claims), placement: taskIntent.placement, coordinator_session_id: taskIntent.coordinator_session_id, participant_session_ids: clone(taskIntent.participant_session_ids), lifecycle: { state: "active" }, attachment_final_digest: payload.attachment_final_digest, operation_intents: 0, tool_completions: 0 });
