...
function assertCurrentAuthorityProjection(value) {
  currentAuthorityRequire(isPlainObject(value) && hasExactKeys(value, ["authority_state_schema_version", "authority_id", "provider", "authority_sequence", "command_ids", "attachments", "tasks"]), "projection fields");
  currentAuthorityRequire(value.authority_state_schema_version === 1 && isUuidV4(value.authority_id) && value.provider === "git_common" && v3Integer(value.authority_sequence, 1), "projection identity");
  ...
  currentAuthorityRequire(Array.isArray(value.attachments) && Array.isArray(value.tasks), "projection collections");
  const attachmentIds = value.attachments.map((item) => item.attachment_id);
  const attachmentTaskIds = value.attachments.map((item) => item.task_id);
  const attachmentAnchorIds = value.attachments.map((item) => item.anchor_id);
  const taskIds = value.tasks.map((item) => item.task_id);
  const taskAttachmentIds = value.tasks.map((item) => item.attachment_id);
  currentAuthorityRequire(new Set(attachmentIds).size === attachmentIds.length, "attachment identity uniqueness");
  currentAuthorityRequire(new Set(attachmentTaskIds).size === attachmentTaskIds.length, "attachment task identity uniqueness");
  ...
  currentAuthorityRequire(new Set(taskAttachmentIds).size === taskAttachmentIds.length, "task attachment uniqueness");
  for (const attachment of value.attachments) {
    currentAuthorityRequire(hasExactKeys(attachment, ["attachment_id", "task_id", "stage_command_id", "lifecycle", "claim_token_digest", "claim_epoch", "anchor_id", "staged_locator_digest", "claimed_locator_digest", "stage_intent_digest", "staged_receipt_digest", "pending_record_digest", "final_record_digest", "goal", "files", "session_id", "placement", "worktree_root", "git_dir", "granted_by", "reason"]), "attachment fields");
    currentAuthorityRequire(isUuidV4(attachment.attachment_id) && isUuidV4(attachment.task_id) && v3NonEmpty(attachment.stage_command_id) && attachment.claim_epoch === 1 && v3NonEmpty(attachment.anchor_id), "attachment identity");
    currentAuthorityRequire(new Set(["staging", "staged", "pending", "claimed"]).has(attachment.lifecycle), "attachment lifecycle");
    ...
    currentAuthorityRequire(v3NonEmpty(attachment.goal) && v3Array(attachment.files, v3RepoPath, { nonEmpty: true }) && v3NonEmpty(attachment.session_id), "attachment task input");
    currentAuthorityRequire(attachment.placement === "partitioned" && v3NonEmpty(attachment.worktree_root) && v3NonEmpty(attachment.git_dir), "attachment placement");
  ...
  for (const task of value.tasks) {
    currentAuthorityRequire(hasExactKeys(task, ["task_id", "attachment_id", "goal", "files", "placement", "session_id", "attachment_final_digest", "operation_intents", "tool_completions"]), "task fields");
    currentAuthorityRequire(isUuidV4(task.task_id) && isUuidV4(task.attachment_id) && v3NonEmpty(task.goal) && v3Array(task.files, v3RepoPath, { nonEmpty: true }), "task identity");
    currentAuthorityRequire(task.placement === "partitioned" && v3NonEmpty(task.session_id) && isSha256Digest(task.attachment_final_digest), "task attachment");
    const owningAttachment = value.attachments.find((item) => item.attachment_id === task.attachment_id) ?? null;
    currentAuthorityRequire(owningAttachment !== null && owningAttachment.task_id === task.task_id, "task attachment reference");
    currentAuthorityRequire(v3Integer(task.operation_intents) && v3Integer(task.tool_completions), "task receipt counters");
  }
  return value;
}
...
function evolveCurrentAuthority(state, record) {
  ...
  if (record.kind === "authority_genesis") {
    ...
    currentAuthorityRequire(isUuidV4(payload.authority_id) && payload.provider === "git_common", "genesis payload");
    return assertCurrentAuthorityProjection({ authority_state_schema_version: 1, authority_id: payload.authority_id, provider: payload.provider, authority_sequence: 1, command_ids: [record.command_id], attachments: [], tasks: [] });
  }
  ...
  if (record.kind === "attachment_stage_intent") {
    currentAuthorityRequire(attachment === null && !next.attachments.some((item) => item.task_id === payload.task_id) && next.attachments.every((item) => item.lifecycle === "claimed"), "stage exclusivity");
    currentAuthorityRequire(!next.attachments.some((item) => item.anchor_id === payload.anchor_id), "attachment anchor uniqueness");
    currentAuthorityRequire(isUuidV4(payload.attachment_id) && isUuidV4(payload.task_id) && isUuidV4(payload.claim_token) && payload.claim_epoch === 1 && v3NonEmpty(payload.anchor_id), "stage identities");
    ...
    currentAuthorityRequire(v3NonEmpty(payload.goal) && v3Array(payload.files, v3RepoPath, { nonEmpty: true }) && v3NonEmpty(payload.session_id), "stage task input");
    currentAuthorityRequire(payload.placement === "partitioned" && v3NonEmpty(payload.worktree_root) && v3NonEmpty(payload.git_dir), "stage placement");
    currentAuthorityRequire(new Set(["self", "user"]).has(payload.granted_by) && v3NonEmpty(payload.reason), "stage provenance");
    next.attachments.push({ attachment_id: payload.attachment_id, task_id: payload.task_id, stage_command_id: record.command_id, lifecycle: "staging", claim_token_digest: sha256Hex(payload.claim_token), claim_epoch: payload.claim_epoch, anchor_id: payload.anchor_id, staged_locator_digest: payload.staged_locator_digest, claimed_locator_digest: payload.claimed_locator_digest, stage_intent_digest: record.record_digest, staged_receipt_digest: null, pending_record_digest: null, final_record_digest: null, goal: payload.goal, files: clone(payload.files), session_id: payload.session_id, placement: payload.placement, worktree_root: payload.worktree_root, git_dir: payload.git_dir, granted_by: payload.granted_by, reason: payload.reason });
    ...
    currentAuthorityRequire(attachment?.lifecycle === "claimed" && attachment.task_id === payload.task_id && payload.attachment_final_digest === attachment.final_record_digest, "task attachment chain");
    currentAuthorityRequire(!next.tasks.some((item) => item.attachment_id === payload.attachment_id || item.task_id === payload.task_id) && payload.goal === attachment.goal && canonicalJson(payload.files) === canonicalJson(attachment.files) && payload.placement === attachment.placement && payload.session_id === attachment.session_id, "task open input");
    next.tasks.push({ ...clone(payload), operation_intents: 0, tool_completions: 0 });
    ...
    currentAuthorityRequire(task && v3NonEmpty(payload.operation_id) && typeof payload.target === "string", "tool receipt route");
    currentAuthorityRequire(v3NonEmpty(payload.session_id) && v3NonEmpty(payload.tool) && (payload.permission_mode === null || typeof payload.permission_mode === "string"), "tool receipt host fields");
...
export {
  ...
  assertCurrentAuthorityProjection,
  machineRiskFloor,
  policyName,
  projectAssurance,
