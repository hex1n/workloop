...
function evolveCurrentAuthority(state, record) {
  ...
  if (record.kind === "attachment_stage_intent") {
    currentAuthorityRequire(attachment === null && next.tasks.length === 0 && next.attachments.every((item) => item.lifecycle === "claimed"), "stage exclusivity");
    ...
    currentAuthorityRequire(next.tasks.length === 0 && payload.goal === attachment.goal && canonicalJson(payload.files) === canonicalJson(attachment.files) && payload.placement === attachment.placement && payload.session_id === attachment.session_id, "task open input");
