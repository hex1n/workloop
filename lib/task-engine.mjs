...
function createTask(facts) {
  ...
  return {
    ...
    goal: facts.goal,
    criterion: { ...clone(facts.criterion), authored_by: facts.criterion.authored_by ?? "self", last_observation: clone(facts.observation) },
    policy,
...
function assertV3DomainEvent(event) {
  ...
  const legacyExpected = event.kind === "task_amended" ? expected.filter((field) => field !== "artifact_revision") : expected;
