...
function machineRiskFloor(task) {
  ...
  const authorityTargets = taskRuntimeContract(task) === 6
    ? Object.values(task.operations ?? {}).flatMap((operation) => operation.authorization?.declared_targets ?? [])
    : touched;
...
function acceptedReview(task, level) {
  ...
  return (task.reviews ?? []).find((review) =>
    ...
    (taskRuntimeContract(task) !== 6 || review.reviewed_evidence_revision === task.evidence.evidence_revision) &&
...
function artifactAssuranceHolds(task) {
  if (taskRuntimeContract(task) !== 6) return [];
  const holds = [];
  ...
  if (task.budget.writes !== null && task.authority.write_operations_authorized > task.budget.writes) holds.push("write_budget_exceeded");
  if (task.history_requirement === "complete") {
    ...
    if (task.authority.prewrite_enforcement !== "full") holds.push("prewrite_enforcement_incomplete");
  }
  return holds;
}
...
function assertHistoryRequirementInvariant(task) {
  ...
  if (task.budget.writes !== null && task.history_requirement !== "complete") {
...
function assertFullMutationHistoryInvariant(task) {
  if (taskRuntimeContract(task) !== 6 || task.evidence.mutation_history_coverage !== "full") return;
  ...
  for (const interval of intervals) {
    const lease = task.capability_leases.find((candidate) => (
      ...
      candidate.effective_from_checkpoint === interval.interval_from_checkpoint
    ));
  ...
  const openAtCurrent = intervals.some((interval) => interval.interval_to_checkpoint === null &&
    ...
    task.capability_leases.some((lease) => lease.operation_id === interval.operation_id && lease.capability_id === interval.capability_id && lease.status === "open"));
...
function invalidateContract6EpisodeCoverage(task) {
  if (taskRuntimeContract(task) !== 6) return;
  const changed = task.evidence.mutation_history_coverage !== "unknown" || task.authority.prewrite_enforcement !== "unknown";
  task.evidence.mutation_history_coverage = "unknown";
  task.authority.prewrite_enforcement = "unknown";
...
function assertContract6Operation(value, operationId) {
  ...
  if (value.completion !== null) {
    exactV3Object(value.completion, ["outcome", "reported_targets", "receipt_quality", "host_profile"], "operation completion");
...
function assertV3TaskProjection(task) {
  ...
  if (!new Set([5, 6]).has(runtimeContract)) throw new Error("invalid schema-v3 task runtime contract");
  ...
  if (runtimeContract === 6) {
    ...
    for (const [operationId, operation] of Object.entries(task.operations)) assertContract6Operation(operation, operationId);
...
function createTask(facts) {
  ...
  if (!new Set([5, 6]).has(runtimeContract)) throw new Error("unsupported task runtime contract");
  ...
  if (runtimeContract === 6) {
    task.runtime_contract = 6;
    task.authority = { write_operations_authorized: 0, prewrite_enforcement: facts.coverageBasis.prewrite_enforcement };
    ...
    task.operations = {};
    task.spent.write_count_basis = "authorized";
    task.evidence = {
      evidence_revision: 0,
...
const V3_DECIDERS = {
  open(state, command) {
    ...
    const payloadVersion = command.runtimeContract === 6 ? 2 : 1;
    const event = v3Event(command, "task_opened", {
      ...
      runtime_contract: payloadVersion === 2 ? 6 : undefined,
  ...
  "complete-tool"(state, command) {
    assertV3CommandState(state, command);
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state === "terminal") throw new Error("complete-tool requires a live Contract 6 task");
    const values = {
      operation_id: command.operationId,
      tool_family: command.toolFamily,
  ...
  "complete-operation"(state, command) {
    assertV3CommandState(state, command);
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state === "terminal") throw new Error("complete-operation requires a live Contract 6 task");
    const completion = {
      outcome: command.outcome,
      reported_targets: [...new Set(command.reportedTargets ?? [])],
      receipt_quality: command.receiptQuality,
  ...
  "reconcile-artifacts"(state, command) {
    assertV3CommandState(state, command);
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state === "terminal") throw new Error("reconcile-artifacts requires a live Contract 6 task");
    const values = {
      checkpoint_id: command.checkpointId,
      from_checkpoint: command.fromCheckpoint,
      to_checkpoint: command.toCheckpoint,
  ...
  "change-coverage"(state, command) {
    assertV3CommandState(state, command);
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state === "terminal") throw new Error("change-coverage requires a live Contract 6 task");
  ...
  observe(state, command) {
    ...
    const observed = v3Event(command, "criterion_observed", {
      ...
      checkpoint_id: taskRuntimeContract(projected) === 6 ? projected.artifact_checkpoint.checkpoint_id : undefined,
      evidence_revision: taskRuntimeContract(projected) === 6 ? projected.evidence.evidence_revision : undefined,
      event_cursor: taskRuntimeContract(projected) === 6 ? projected.task_event_sequence + 1 : undefined,
    }, state.task_id, taskRuntimeContract(projected) === 6 ? 2 : 1);
  ...
  resume(state, command) {
    ...
    if (taskRuntimeContract(state) === 6 && state.capability_leases.some((lease) => lease.status === "open")) throw new Error("resume cannot rotate an episode with an open operation lease");
  ...
  join(state, command) {
    ...
    if (taskRuntimeContract(state) === 6 && state.capability_leases.some((lease) => lease.status === "open")) throw new Error("join cannot rotate an episode with an open operation lease");
  ...
  amend(state, command) {
    ...
    const values = {
      ...
      ...(taskRuntimeContract(state) === 6 ? { history_requirement: command.historyRequirement ?? null } : {}),
    };
    const event = v3Event(command, "task_amended", values, state.task_id, taskRuntimeContract(state) === 6 ? 2 : 1);
    V3_REDUCERS.task_amended(clone(state), event);
    return { events: [event], result: { status: "amended" } };
  },
  "criterion-side-effect"(state, command) {
    ...
    events.push(v3Event(command, "criterion_side_effect_recorded", {
      observation: command.observation,
      checkpoint_id: taskRuntimeContract(state) === 6 ? projected.artifact_checkpoint.checkpoint_id : undefined,
      evidence_revision: taskRuntimeContract(state) === 6 ? projected.evidence.evidence_revision : undefined,
      event_cursor: taskRuntimeContract(state) === 6 ? state.task_event_sequence + events.length + 1 : undefined,
    }, state.task_id, taskRuntimeContract(state) === 6 ? 2 : 1));
  ...
  achieve(state, command) {
    ...
    const observed = v3Event(command, "criterion_observed", {
      ...
      checkpoint_id: taskRuntimeContract(state) === 6 ? state.artifact_checkpoint.checkpoint_id : undefined,
      evidence_revision: taskRuntimeContract(state) === 6 ? state.evidence.evidence_revision : undefined,
      event_cursor: taskRuntimeContract(state) === 6 ? state.task_event_sequence + 1 : undefined,
    }, state.task_id, taskRuntimeContract(state) === 6 ? 2 : 1);
...
const V3_REDUCERS = {
  ...
  tool_completed(state, event) {
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state === "terminal") throw new Error("tool_completed requires a live Contract 6 task");
    const payload = event.payload;
    ...
    const operation = state.operations[payload.operation_id] ?? { operation_id: payload.operation_id, authorization: null, completion: null };
  ...
  artifact_reconciled(state, event) {
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state === "terminal") throw new Error("artifact_reconciled requires a live Contract 6 task");
    const payload = event.payload;
    ...
    if (payload.to_checkpoint !== payload.from_checkpoint) {
      ...
      const nextEntries = [...entries].sort(([left], [right]) => left.localeCompare(right)).map(([entryPath, entry]) => ({ path: entryPath, ...entry }));
  ...
  coverage_changed(state, event) {
    if (taskRuntimeContract(state) !== 6 || state.lifecycle?.state === "terminal") throw new Error("coverage_changed requires a live Contract 6 task");
    const payload = event.payload;
    const currentEpisode = state.episodes.at(-1);
    ...
    const prior = {
      ...
      prewrite_enforcement: state.authority.prewrite_enforcement,
    };
    ...
    state.authority.prewrite_enforcement = payload.prewrite_enforcement;
  ...
  task_resumed(state, event) {
    ...
    if (taskRuntimeContract(state) === 6 && state.capability_leases.some((lease) => lease.status === "open")) throw new Error("task_resumed cannot rotate an episode with an open operation lease");
    ...
    invalidateContract6EpisodeCoverage(state);
  ...
  task_joined(state, event) {
    ...
    if (taskRuntimeContract(state) === 6 && state.capability_leases.some((lease) => lease.status === "open")) throw new Error("task_joined cannot rotate an episode with an open operation lease");
    nextRevision(state);
    closeCurrentEpisode(state, event.at);
    state.episodes.push(episode);
    invalidateContract6EpisodeCoverage(state);
  ...
  criterion_side_effect_recorded(state, event) {
    ...
    nextRevision(state, taskRuntimeContract(state) === 6 ? { substantive: true } : { artifact: true });
