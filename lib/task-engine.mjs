...
function machineRiskFloor(task) {
  ...
  if (touched.includes("<command>")) { risk = riskMax(risk, "substantial"); reasons.push("unattributed_write"); }
  if (touched.includes("<command:publish>") || touched.includes("<command:git_push>")) { risk = "critical"; reasons.push("irreversible_authority_used"); }
  if (touched.includes("<command:destructive>")) { risk = riskMax(risk, "substantial"); reasons.push("destructive_command_used"); }
...
function assertArtifactCheckpoint(value, label) {
  exactV3Object(value, ["checkpoint_id", "entries"], label);
  v3Require(v3Digest(value.checkpoint_id), `${label} checkpoint_id`);
  v3Require(v3Array(value.entries, (entry) => {
    try {
      exactV3Object(entry, ["path", "hash"], `${label} entry`);
      return v3RepoPath(entry.path) && v3Digest(entry.hash);
    } catch { return false; }
  }), `${label} entries`);
...
const V3_DECIDERS = {
  ...
  "complete-operation"(state, command) {
    ...
    const events = [];
    if (!duplicate && !conflicting) events.push(v3Event(command, "tool_completed", {
      operation_id: command.operationId,
      tool_family: command.toolFamily,
      ...completion,
    }, state.task_id));
    ...
    if (coverage) events.push(v3Event(command, "coverage_changed", {
      artifact_state: coverage.artifactState,
      mutation_history: conflicting ? "unknown" : coverage.mutationHistory,
      prewrite_enforcement: conflicting ? "unknown" : coverage.prewriteEnforcement,
      ...
      reason: conflicting ? `operation receipt conflict: ${coverage.reason}` : coverage.reason,
    }, state.task_id));
    return { events, result: { status: conflicting ? "conflict" : duplicate ? "duplicate" : "completed" } };
  },
  "reconcile-artifacts"(state, command) {
    ...
    if (command.coverageChange) {
      const coverage = command.coverageChange;
      events.push(v3Event(command, "coverage_changed", {
        artifact_state: coverage.artifactState,
  ...
  "change-coverage"(state, command) {
    ...
    const values = {
      artifact_state: command.artifactState,
      mutation_history: command.mutationHistory,
      prewrite_enforcement: command.prewriteEnforcement,
      episode_id: command.episodeId,
      operation_id: command.operationId ?? null,
      capability_id: command.capabilityId ?? null,
      host_profile: command.hostProfile,
      surface: command.surface,
      exhaustive_surface: command.exhaustiveSurface,
      effective_from_checkpoint: command.effectiveFromCheckpoint,
      interval_from_checkpoint: command.intervalFromCheckpoint,
      interval_to_checkpoint: command.intervalToCheckpoint ?? null,
      reason: command.reason,
    };
    return { events: [v3Event(command, "coverage_changed", values, state.task_id)], result: { status: "coverage_changed" } };
  },
...
const V3_REDUCERS = {
  ...
  artifact_reconciled(state, event) {
    ...
    if (payload.to_checkpoint !== payload.from_checkpoint) {
      const entries = new Map(state.artifact_checkpoint.entries.map((entry) => [entry.path, entry.hash]));
      for (const change of payload.changed_entries) {
        const before = entries.get(change.path) ?? null;
        if (before !== change.before) throw new Error("artifact_reconciled delta does not match current checkpoint");
        if (change.after === null) entries.delete(change.path);
        else entries.set(change.path, change.after);
      }
      const nextEntries = [...entries].sort(([left], [right]) => left.localeCompare(right)).map(([entryPath, hash]) => ({ path: entryPath, hash }));
      ...
      state.artifact_checkpoint = { checkpoint_id: payload.to_checkpoint, entries: nextEntries };
    ...
    state.evidence.current_scope_violations = [...new Set(payload.current_scope_violations)].sort();
  ...
  coverage_changed(state, event) {
    ...
    if (payload.operation_id !== null && payload.exhaustive_surface) {
      ...
      if (payload.interval_to_checkpoint === null) {
        ...
        if (!existing || existing.status !== "open" || existing.capability_id !== payload.capability_id || existing.effective_from_checkpoint !== payload.interval_from_checkpoint) {
...
function evolveInto(state, event) {
  ...
  if (runtimeContract === 6) {
    const expectedVersion = new Set(["write_authorized", "criterion_observed", "criterion_side_effect_recorded", "review_recorded"]).has(event.kind) ? 2 : 1;
