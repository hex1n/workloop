...
function assertCurrentAuthorityProjection(value) {
  currentAuthorityRequire(isPlainObject(value) && hasExactKeys(value, ["authority_state_schema_version", "authority_id", "provider", "authority_sequence", "command_ids", "attachments", "placement_intents", "task_intents", "tasks"]), "projection fields");
  ...
  for (const intent of value.task_intents) {
    currentAuthorityRequire(hasExactKeys(intent, ["task_id", "attachment_id", "command_id", "goal", "write_claims", "placement", "branch_intent", "base_oid", "placement_intent_digest", "coordinator_session_id", "participant_session_ids", "status", "intent_record_digest"]), "task intent fields");
  ...
  for (const task of value.tasks) {
    currentAuthorityRequire(hasExactKeys(task, ["task_id", "attachment_id", "goal", "write_claims", "placement", "branch_intent", "base_oid", "placement_intent_digest", "coordinator_session_id", "participant_session_ids", "lifecycle", "attachment_final_digest", "operation_intents", "tool_completions"]), "task fields");
...
function evolveCurrentAuthority(state, record) {
  ...
  if (record.kind === "authority_genesis") {
    ...
    return assertCurrentAuthorityProjection({ authority_state_schema_version: 1, authority_id: payload.authority_id, provider: payload.provider, authority_sequence: 1, command_ids: [record.command_id], attachments: [], placement_intents: [], task_intents: [], tasks: [] });
  }
