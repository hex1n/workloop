...
import { STATE_DIR, TASK_FILE, V3_TASK_SNAPSHOT_SCHEMA_VERSION, canonicalJson, foldCasePath, globToRegExp, hasExactKeys, isPlainObject, sha256Hex, userHome } from "./prims.mjs";
...
function digestVerifiedV3Projection(stored) {
  ...
  const expectedRuntimeContract = stored?.projection?.runtime_contract === 7 ? 6 : stored?.projection?.runtime_contract === 6 ? 5 : 4;
  if (
    stored.runtime_contract !== expectedRuntimeContract ||
