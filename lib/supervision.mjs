...
function controlPlaneWriteFailure(repo, tool, mapping, home = os.homedir(), callAnalysis = null) {
  const call = callAnalysis ?? analyzeToolCall(tool, mapping);
  const targets = writeFileTargets(tool, mapping, call);
...
function foreignAnalysisFailure(analysis) {
  const hasEffect = (effect) => analysis.effects.includes(effect);
  if (hasEffect("publish")) return "foreign session irreversible publication is denied; use taskloop join";
  if (hasEffect("dynamic_exec")) return "foreign session dynamic interpreter input is denied; use taskloop join";
  if (analysis.network.unresolvedWrite) return "foreign session remote output is not provably stdout-only; use taskloop join";
  if (hasEffect("remote_exec")) return "foreign session remote-exec is denied; use taskloop join";
  if (hasEffect("install")) return "foreign session package installation is denied; use taskloop join";
  if (hasEffect("secret_dump")) return "foreign session secret dump is denied; use taskloop join";
  if (hasEffect("destructive")) return "foreign session destructive command is denied; use taskloop join";
  if (analysis.git.subcommands.length > 0 && !analysis.git.readonly) return "foreign session git command is not read-only; use taskloop join";
  return null;
}

function foreignWriteDecision(repo, task, tool, mapping, callAnalysis = null) {
  const call = callAnalysis ?? analyzeToolCall(tool, mapping);
  for (const { analysis } of call.commands) {
    const failure = foreignAnalysisFailure(analysis); if (failure) return { kind: "deny", message: failure };
  }
  const targets = writeFileTargets(tool, mapping, call);
  ...
  const targetlessWrite = looksLikeWrite(tool, mapping, call) && targets.length === 0;
