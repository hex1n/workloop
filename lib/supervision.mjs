...
// Windows-rooted paths, whatever the dialect. Four shapes reach real files and a
// POSIX lexer/normalizer mangles all four:
//   drive root      C:\x
//   drive-relative  C:x        (that drive's current directory — the repo, for
//                               an agent working in-repo)
//   UNC root        \\server\share, \\?\C:\x, \\localhost\C$\x
//   rooted          \dir\file  (root of the current drive; win32.isAbsolute
//                               reports true, and it names the same file a
//                               drive-letter path does)
// The rooted form requires a later separator so a POSIX escape of a command
// word (`\ls` defeating an alias) is not mistaken for a path — misreading a
// command word would break command classification, which fails toward allow.
// The drive form rejects a second colon so ordinary POSIX text with a
// letter-colon prefix (`s:/old:/new:` for sed, a one-letter scp host alias)
// keeps normal escape processing.
function windowsRootedPath(value) {
  const raw = String(value ?? "");
  if (/^\\\\[^\\/]/.test(raw)) return true;
  if (/^\\[^\\/][^\s]*[\\/]/.test(raw)) return true;
  if (!/^[A-Za-z]:/.test(raw)) return false;
  // A drive path may carry one NTFS alternate-data-stream suffix — a write to
  // `...\.git\config:hidden` writes the same file entry as `config`, so it must
  // reach the guard. Two colons is therefore still a path; three or more is
  // sed-style syntax (`s:/old:/new:`) that must keep POSIX escaping.
...
function readShellValue(source, start, dialect) {
  ...
  // A POSIX lexer eats every backslash in `C:\Users\...\.git\config`, leaving
  // `C:Users....gitconfig` — a name that resolves nowhere near the control
  // plane, so the guard that protects .git and .workloop silently passes it.
  // Hosts hand us Windows-shaped absolute paths through POSIX-dialect tool
  // calls routinely, so a drive-letter word keeps its literal text. Consistent
  // with the rest of this file, the ambiguity resolves toward a false deny (a
  // caller asked to write a literal `C:Users...` name) over a false allow (a
  // write to control state that was never seen).
  // UNC counts too: `\\server\share` and especially `\\localhost\C$\...` reach
  // the same files a drive letter does, so covering only `C:\` would leave an
  // alternate spelling of every protected path unguarded.
  const raw = source.slice(start, index);
  if (dialect === "posix" && windowsRootedPath(raw)) {
    return { value: raw, end: index, started };
  }
...
function resolveShellTarget(cwd, target, dialect) {
  ...
  // A drive-letter path is Windows-shaped whatever the dialect: hosts hand
  // POSIX-dialect tool calls native absolute paths routinely. POSIX rules treat
  // `C:\a\b` as a single segment, so one `..` erases the entire prefix and
  // `C:\a\repoB/../repoA/x` collapses to the relative `repoA/x`, which then
  // re-resolves against the repo and lands nowhere near the real target.
  const windowsShaped = windowsRootedPath(value) || windowsRootedPath(cwd);
  const windows = dialect === "cmd" || dialect === "powershell" || windowsShaped;
