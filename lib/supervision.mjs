...
function commandExecutionViews(command) {
  const raw = String(command);
  const views = [stripQuotedSegments(commandSyntaxOnly(raw))];
  let singleQuoted = false; let doubleQuoted = false;
  for (let index = 0; index < raw.length; index += 1) {
    const char = raw[index];
    if (char === "'" && !doubleQuoted && raw[index - 1] !== "\\") { singleQuoted = !singleQuoted; continue; }
    if (char === '"' && !singleQuoted && raw[index - 1] !== "\\") { doubleQuoted = !doubleQuoted; continue; }
    if (singleQuoted) continue;
    if (char === "$" && raw[index + 1] === "(") {
      let depth = 1; let cursor = index + 2;
      for (; cursor < raw.length && depth; cursor += 1) {
        if (raw[cursor] === "(" && raw[cursor - 1] !== "\\") depth += 1;
        if (raw[cursor] === ")" && raw[cursor - 1] !== "\\") depth -= 1;
      }
      if (depth === 0) { views.push(raw.slice(index + 2, cursor - 1)); index = cursor - 1; }
      continue;
    }
    if (char === "`") {
      const end = raw.indexOf("`", index + 1);
      if (end > index) { views.push(raw.slice(index + 1, end)); index = end; }
    }
  }
  // Keep the quoted-body alternatives disjoint. Allowing the fallback class
  // to consume a backslash makes an unterminated `sh -c "\\...` payload
  // exponentially expensive to reject.
  for (const match of raw.matchAll(/(?:^|[;&|(\r\n]\s*)(?:[^\s;&|()]*\/)?(?:bash|sh|zsh)\s+-[A-Za-z]*c[A-Za-z]*\s+(?:"((?:\\.|[^"\\])*)"|'([^']*)')/gi)) {
    // Decode only the characters for which a POSIX shell consumes a
    // backslash inside double quotes. Single-quoted bodies are already exact.
    views.push(match[1] !== undefined ? match[1].replace(/\\\r?\n/g, "").replace(/\\(["\\$`])/g, "$1") : (match[2] ?? ""));
  }
  for (const match of raw.matchAll(/(?:^|[;&|(\r\n]\s*)(?:[^\s;&|()]*\/)?(?:bash|sh|zsh)\b[^\r\n]*?<<-?\s*["']?([A-Za-z0-9_.-]+)["']?[^\r\n]*\r?\n([\s\S]*?)\r?\n[ \t]*\1(?=\s|$)/gi)) {
    views.push(match[2]);
  }
...
function commandShapes(command) {
  const raw = String(command);
  const views = commandExecutionViews(raw);
  const shapes = [];
  if (views.some(publishShape)) shapes.push("publish");
  if (views.some((value) => /\bgit\s+(?:-[^\s]+\s+)*push\b/i.test(value))) shapes.push("git_push");
  if (views.some((value) => /\br(?:m)\s+(-\S*[rf]|--(?:recursive|force|dir))|\bf(?:ind)\b[^|]*\s-delete\b|\b(DR(?:OP)\s+TABLE|TRUN(?:CATE)|DEL(?:ETE)\s+FROM)\b/i.test(value))) shapes.push("destructive");
  if (views.some((value) => /\b(c(?:url)|w(?:get)|Invoke-WebRe(?:quest))\b/i.test(value))) shapes.push("network");
  if (views.some((value) => /\bw(?:get)\b/i.test(value) || curlRemoteName(value) || /\bInvoke-WebRe(?:quest)\b[^;&|]*\s-OutF(?:ile)(?:\s|:)/i.test(value)) || (/\bc(?:url)\b/i.test(raw) && writeFileTargets("", { command: raw }).length > 0)) shapes.push("network_write");
  if (views.some((value) => /\b(npm|pnpm|yarn|bun)\s+(i|install|add)\b|\bpip3?\s+install\b/i.test(value))) shapes.push("install");
...
function foreignCommandFailure(command) {
  ...
  if (publishShape(value)) return "foreign session irreversible publication is denied; use taskloop join";
  if (/\bInvoke-WebRe(?:quest)\b[^;&|]*\s-OutF(?:ile)(?:\s|:)/i.test(value)) return "foreign session remote output is not provably stdout-only; use taskloop join";
  ...
  for (const field of out.split("\0")) {
    ...
    try {
      ...
      if (stored?.schema_version === V3_TASK_SNAPSHOT_SCHEMA_VERSION) {
        const fields = ["schema_version", "runtime_contract", "source_cursor", "projection", "snapshot_digest"];
        if (
          stored.runtime_contract !== V3_RUNTIME_CONTRACT ||
          JSON.stringify(Object.keys(stored).sort()) !== JSON.stringify(fields.sort()) ||
          !isPlainObject(stored.source_cursor) || !isPlainObject(stored.projection)
        ) continue;
        const preimage = {
          schema_version: stored.schema_version,
          runtime_contract: stored.runtime_contract,
          source_cursor: stored.source_cursor,
          projection: stored.projection,
        };
        if (sha256Hex(canonicalJson(preimage)) !== stored.snapshot_digest) continue;
        task = stored.projection;
