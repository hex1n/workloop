...
import { STATE_DIR, TASK_FILE, V3_RUNTIME_CONTRACT, V3_TASK_SNAPSHOT_SCHEMA_VERSION, canonicalJson, globToRegExp, isPlainObject, repoRelative, sha256Hex } from "./prims.mjs";
...
// exempting by the segment's command-position tool costs no reach.
const TEXT_ONLY_TOOLS = new Set(["echo", "printf", "cat", "grep", "rg", "sed", "awk", "head", "tail", "less", "more", "ls", "man", "which", "test"]);
...
function scopedDestructiveFailure(task, analysis, options) {
  const escalation = "run taskloop amend --destructive-allowed --reason <why> for full destructive authority";
...
function commandSafetyFailure(task, command, options = {}) {
  ...
  if (!env.network && hasEffect("network")) {
    return "network command requires an explicit network grant; run taskloop amend --network-allowed --reason <why> to request one";
  }
  if (!hasGrant("install") && hasEffect("install")) {
    return "package install requires an explicit install grant; run taskloop amend --install-scripts-allowed --reason <why> to request one";
  }
  if (!hasGrant("publish") && hasEffect("publish")) {
    return "publish-shaped command requires an explicit publish grant; run taskloop amend --publish-allowed --reason <why> to request one";
  }
  ...
  if (!env.destructive && hasEffect("destructive")) {
    if (!hasGrant("destructive")) {
      return "destructive command requires an explicit destructive grant; run taskloop amend --destructive-scope <root> --reason <why> for transient-tree cleanup, --destructive-allowed for full authority, or leave transient files for the host to reap";
    }
...
function controlPlaneWriteFailure(repo, tool, mapping, home = os.homedir(), callAnalysis = null) {
  const call = callAnalysis ?? analyzeToolCall(tool, mapping);
...
function foreignAnalysisFailure(analysis) {
  ...
  if (hasEffect("publish")) return host("irreversible publication needs task authorization; run taskloop join to act under the active task");
  if (hasEffect("dynamic_exec")) return host("dynamically constructed interpreter input cannot be statically checked; run taskloop join to act under the active task");
  if (analysis.network.unresolvedWrite) return host("remote output is not provably stdout-only; run taskloop join to act under the active task");
  if (hasEffect("remote_exec")) return host("remote-exec needs task authorization; run taskloop join to act under the active task");
  if (hasEffect("install")) return host("package installation needs task authorization; run taskloop join to act under the active task");
  if (hasEffect("secret_dump")) return host("secret exfiltration needs task authorization; run taskloop join to act under the active task");
  if (hasEffect("destructive")) return host("destructive command needs task authorization; run taskloop join to act under the active task");
  if (analysis.git.subcommands.length > 0 && !analysis.git.readonly) return { category: "git", message: "non-read-only git needs task authorization; run taskloop join to act under the active task" };
  return null;
}
...
// no claim over it, so the target's own taskloop state should decide. This
...
function foldedEnvelopeGlobs(files) {
  return files.map((pattern) => foldPath(String(pattern)));
}
...
function digestVerifiedV3Projection(stored) {
  ...
  if (
    ...
    JSON.stringify(Object.keys(stored).sort()) !== JSON.stringify(fields) ||
...
// null: no taskloop state (unsupervised). undefined: state present but not
...
function gitExternalDelegation(repo, command, analysis, sessionId) {
  ...
  if (task?.unreadable) return { kind: "deny", message: `cannot read the taskloop state of ${parsed.chdir}; run the command from that repository` };
  if (!task || ownedBySession(task, sessionId)) return { kind: "allow" };
  return { kind: "deny", message: `${parsed.chdir} has an active taskloop task (task ${task.task_id}); join it in that repository or run under its owning session` };
}
...
function externalTargetDecision(raw, absoluteTarget, sessionId, home) {
  ...
  const controlStateDenied = { kind: "deny", message: `direct writes to taskloop/git control state are denied: ${raw}` };
  ...
  if (task?.unreadable) return { kind: "deny", message: `cannot read the taskloop state of ${repoRoot}; run the command from that repository` };
  if (!task || ownedBySession(task, sessionId)) return { kind: "allow" };
...
function foreignWriteDecision(repo, task, tool, mapping, callAnalysis = null, home = os.homedir(), sessionId = null) {
  const call = callAnalysis ?? analyzeToolCall(tool, mapping);
  for (const { command, analysis } of call.commands) {
    ...
    // that repository's taskloop state, not this one's floor.
  ...
  // Category (2): taskloop cannot prove where the write lands, so it cannot rule
  ...
  if (call.commands.some(({ command }) => /(?:^|[;&|()]\s*)cd\s+[^;&|]+/i.test(command)) && targets.some((target) => !path.isAbsolute(String(target)))) {
    return { kind: "deny", message: "cannot resolve the write target: it depends on a shell directory change; use an absolute path or split the command, or run taskloop join" };
  }
  ...
  if (!destinations.resolved) return { kind: "deny", message: "cannot resolve the write target; use an absolute path or split the command, or run taskloop join" };
  if (looksLikeWrite(tool, mapping, call) && targets.length === 0) return { kind: "deny", message: "cannot resolve the write target; use an absolute path or split the command, or run taskloop join" };
  const canonicalRepo = canonicalPath(repo);
  ...
  if (normalized.some((row) => !row.absolute)) return { kind: "deny", message: "cannot safely resolve the write target (variable, glob, or unsafe path); use an absolute path, or run taskloop join" };
  ...
  for (const row of normalized) {
    if (controlRoots.some((root) => pathInside(row.absolute, root))) return { kind: "deny", message: `direct writes to taskloop/git control state are denied: ${row.raw}` };
    ...
    if (insideEnvelope(relative, envelopeGlobs) || envelopeRegionContains(relative, envelopeGlobs)) {
      return { kind: "deny", message: `write to ${row.raw} conflicts with the active task envelope (task ${task.task_id}); run taskloop join to take over, or use a separate worktree for parallel work` };
    }
  }
