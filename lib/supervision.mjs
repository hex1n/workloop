...
// state === "open" covers active and suspended (suspend does not change state).
function realPath(p) {
  try {
...
function siblingWorktreeOpenTasks(repo) {
  ...
  for (const field of out.split("\0")) {
    ...
    try {
      ...
      if (isPlainObject(task) && task.state === "open") {
        tasks.push({
          path: wt,
          ...
          opened_at: task.spent?.opened_at ?? null,
          suspended: isPlainObject(task.suspension) ? (task.suspension.outcome ?? "suspended") : null,
        });
      }
    } catch {
