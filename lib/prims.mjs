...
const RUNTIME_CONTRACT = 5;
const CRITERION_ADAPTER_PROTOCOL_VERSION = 2;

// Runtime contract 5 changes active artifact names only. Keep these persisted
// schema versions independent: snapshots and HOME projections may evolve
// without changing the append-only record framing.
...
function releaseOwnedDirectoryLock(lock, token, { attempts = 20, fsOps = fs, pathExists = null, readOwner = null, wait = () => {} } = {}) {
  const lockExists = pathExists ?? ((target) => fsOps.existsSync(target));
  const ownerOf = readOwner ?? ((target) => readJsonLockOwner(target, fsOps));
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    if (!lockExists(lock)) return;
...
function reapStaleDirectoryLock(lock, { staleMs, fsOps = fs, now = Date.now } = {}) {
...
// The one cross-process mkdir-lock implementation. Callers own only policy:
...
function withOwnedDirectoryLock(lock, action, {
  ...
  for (;;) {
    try {
      fsOps.mkdirSync(lock); // atomic: throws EEXIST while the lock is held
    } catch (error) {
      if (error?.code !== "EEXIST") throw error;
      if (reapStaleDirectoryLock(lock, { staleMs, fsOps, now })) continue;
      if (now() >= deadline) throw timeoutError(timeoutMs);
      wait();
      continue;
    }
    try {
      fsOps.writeFileSync(path.join(lock, "owner.json"), JSON.stringify({ pid: process.pid, token, ...(ownerExtra ?? {}) }));
    } catch (error) {
      if (removeOnOwnerWriteFailure) {
        try { fsOps.rmSync(lock, { recursive: true, force: true }); } catch { /* preserve the original acquisition error */ }
      }
      throw error;
    }
    try {
      ...
      try {
        releaseOwnedDirectoryLock(lock, token, { fsOps, wait });
      } catch (error) {
