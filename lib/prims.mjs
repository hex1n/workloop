...
// The single time rendering for everything taskloop writes: local wall-clock,
// `YYYY-MM-DD HH:MM:SS`, no `T`/`Z`/offset. V8 parses this back as local time,
// so every `Date.parse`/diff against `Date.now()` stays correct; the exact
// instant for machine-critical leases rides the numeric `*_ms` fields, not this
// string. A filename drops the space/colon to hyphens (no OS forbids those).
function localStamp(when = new Date()) {
