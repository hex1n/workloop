...
const EVENT_PAYLOAD_FIELDS_BY_VERSION = Object.freeze({
  ...
  task_amended: Object.freeze({ 1: V3_EVENT_PAYLOAD_FIELDS.task_amended }),
  ...
  artifact_reconciled: Object.freeze({
    1: Object.freeze(["checkpoint_id", "from_checkpoint", "to_checkpoint", "changed_entries", "changed_paths", "current_scope_violations", "coverage", "reason"]),
  }),
...
function releaseOwnedDirectoryLock(lock, token, { attempts = 20, fsOps = fs, pathExists = null, readOwner = null, wait = () => {} } = {}) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    if (owner?.token !== token) {
      if (owner || attempt === attempts - 1) break;
      wait();
      continue;
    }
...
function withOwnedDirectoryLock(lock, action, {
  ...
  for (;;) {
    ...
    try {
      ...
      try { releaseOwnedDirectoryLock(lock, token, { fsOps, wait, pathExists: () => false }); } catch { /* preserve acquisition error */ }
      throw error;
    }
    try {
      return action();
