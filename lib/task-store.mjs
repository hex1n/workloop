...
// `source` defaults to the task snapshot but the caller may name any control
// artifact, because unreadable state is not only a legacy snapshot: an event
// record the reducer rejects is equally unloadable and equally in need of a
// user-authorized archive. The assembly module picks which artifact holds the
// damage; this stays a leaf that archives whatever it is handed.
function archiveIncompatibleState(repo, { reason, grantedBy, at, source = taskPath(repo) }) {
