...
import { STATE_DIR, TASK_FILE, TASK_SCHEMA_VERSION, fnv1aHex, isPlainObject } from "./prims.mjs";
...
function withTaskLock(repo, action, { timeoutMs } = {}) {
  ...
  for (;;) {
    ...
    try {
      fs.writeFileSync(path.join(lock, "owner.json"), JSON.stringify({ pid: process.pid, token, at: Date.now() }));
      return action();
    } finally {
...
// Make an ISO timestamp filename-safe. A random suffix prevents two archival
// attempts for the same task and timestamp from overwriting one another.
function archiveStamp(at) {
  return String(at).replace(/[^0-9A-Za-z.-]/g, "-");
}
...
function archiveTask(repo, task, at) {
  ...
  fs.writeFileSync(
    path.join(archive, `task-${archiveStamp(at)}-${task.task_id ?? "unknown"}-${randomUUID()}.json`),
...
function archiveIncompatibleState(repo, { reason, grantedBy, at }) {
  ...
  fs.mkdirSync(dir, { recursive: true });
  const stamp = String(at).replace(/[^0-9]/g, "").slice(0, 14);
  const archive = path.join(dir, `incompatible-${stamp}-${digest}.json`);
  if (fs.existsSync(archive)) throw new Error(`archive target already exists: ${archive}`);
