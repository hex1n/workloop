...
// The history filename reuses the task's own local stamp (`YYYY-MM-DD HH:MM:SS`),
// collapsing the space and colons to hyphens — the only chars an OS forbids in a
// filename. One separator throughout, no `T`/`Z`/`:`, still readable in the
// archiver's own timezone.
...
function archiveTask(repo, task, at) {
  ...
  fs.writeFileSync(
    path.join(archive, `task-${archiveStamp(at)}-${task.id ?? "legacy"}.json`),
