...
const CRITERION_LEASE_ACQUIRE_TIMEOUT_MS = 25;
const CRITERION_LEASE_ACQUIRE_GRACE_MS = 5000;
const CRITERION_LEASE_CLEANUP_MARGIN_MS = 10_000;
...
function criterionLeasePath(repo) {
  return path.join(repo, STATE_DIR, ".criterion.lock");
}

function readCriterionLease(repo, { fsOps = fs } = {}) {
  try {
    const owner = JSON.parse(fsOps.readFileSync(path.join(criterionLeasePath(repo), "owner.json"), "utf8"));
    return isPlainObject(owner) ? owner : null;
  } catch {
    return null;
  }
}

function withCriterionLease(repo, lease, action, {
  timeoutMs = CRITERION_LEASE_ACQUIRE_TIMEOUT_MS,
  cleanupMarginMs = CRITERION_LEASE_CLEANUP_MARGIN_MS,
  fsOps = fs,
  now = Date.now,
} = {}) {
  const startedAtEpochMs = lease?.started_at_epoch_ms;
  const deadlineEpochMs = lease?.deadline_epoch_ms;
  if (!Number.isSafeInteger(startedAtEpochMs) || !Number.isSafeInteger(deadlineEpochMs) || deadlineEpochMs <= startedAtEpochMs) {
    throw new Error("criterion lease requires an increasing integer deadline");
  }
  const lock = criterionLeasePath(repo);
  fsOps.mkdirSync(path.dirname(lock), { recursive: true });
  try {
    const value = withOwnedDirectoryLock(lock, action, {
      timeoutMs,
      staleMs: CRITERION_LEASE_ACQUIRE_GRACE_MS,
      fsOps,
      now,
      ownerExtra: lease,
      staleOwner: (owner, clock) => owner === null || (
        Number.isSafeInteger(owner?.deadline_epoch_ms)
        && clock.nowEpochMs > owner.deadline_epoch_ms + cleanupMarginMs
      ),
      timeoutError: (limit) => Object.assign(
        new Error(`workloop: criterion already running after ${limit}ms`),
        { code: "CRITERION_IN_PROGRESS" },
      ),
    });
    return { status: "acquired", value };
  } catch (error) {
    if (error?.code !== "CRITERION_IN_PROGRESS") throw error;
    return { status: "busy", lease: readCriterionLease(repo, { fsOps }) };
  }
}
...
function persistedTaskRuntimeContract(projection) {
  return projection?.runtime_contract === 7 ? 6 : projection?.runtime_contract === 6 ? 5 : 4;
}

function snapshotPreimage(sourceCursor, projection, runtimeContract = persistedTaskRuntimeContract(projection)) {
  return {
...
function validateTaskSnapshot(snapshot, { validateProjection } = {}) {
  ...
  if (snapshot.schema_version !== V3_TASK_SNAPSHOT_SCHEMA_VERSION || snapshot.runtime_contract !== persistedTaskRuntimeContract(snapshot.projection)) throw new Error("invalid schema-v3 snapshot contract");
  ...
  const preimage = snapshotPreimage(snapshot.source_cursor, snapshot.projection, snapshot.runtime_contract);
...
function inspectTaskSnapshot(repo, { fsOps = fs, validateProjection = null } = {}) {
  ...
  try {
    const expected = sha256Hex(canonicalJson(snapshotPreimage(snapshot.source_cursor, snapshot.projection, snapshot.runtime_contract)));
