...
function incrementalAdditions(repoIdentity, records, target, home, operations = fs) {
  ...
  if (cursor.last_repo_sequence === record.repo_sequence && cursor.last_record_digest === record.record_digest) return [];
  if (
    record.repo_sequence !== cursor.last_repo_sequence + 1 ||
    record.previous_record_digest !== cursor.last_record_digest
  ) return null;
  return rowsForRecord(repoIdentity, record);
}
...
function syncOutcomeRecords({ repoIdentity, records, priorRecords = null, home, fsOps = fs, incremental = false }) {
  ...
  const fastAdditions = incremental ? incrementalAdditions(repoIdentity, records, target, home, fsOps) : null;
  let existing = null;
  let additions = fastAdditions;
  if (additions === null) {
