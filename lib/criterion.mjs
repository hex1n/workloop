  ...
  for (const file of files) {
    ...
    try {
      ...
      if (reused) entries.push(reused);
        ...
        entries.push(`${file}\0${contentHash}`);
      }
      assertSnapshotDeadline(deadlineEpochMs, now);
    } catch (error) {
  ...
  for (const file of listed.files) {
    ...
    try {
      ...
      if (stat.isSymbolicLink()) {
        const entry = `${file}\0${sha256Hex(`link:${fs.readlinkSync(absolute)}`)}`;
        ...
        if (expected.stats?.get(file) !== key) changed.add(file);
          ...
          const entry = `${file}\0${hashFileForSnapshot(absolute, { deadlineEpochMs, now })}`;
          if (entry !== expectedEntries.get(file)) changed.add(file);
        }
      }
      assertSnapshotDeadline(deadlineEpochMs, now);
...
function killTree() {
  ...
  if (process.platform === "win32") {
    try { spawnSync("taskkill.exe", ["/pid", String(child.pid), "/t", "/f"], { windowsHide: true, stdio: "ignore" }); } catch {}
  } else {
...
try {
  ...
  const timer = setTimeout(() => { timedOut = true; killTree(); }, spec.timeout_ms);
  child.on("close", (status, signal) => {
    ...
    const meta = {
      ...
      signal,
      timed_out: timedOut,
      error_code: timedOut ? "ETIMEDOUT" : overflow ? "ENOBUFS" : spawnError?.code ?? null,
    };
    process.stderr.write("\n" + spec.marker + JSON.stringify(meta) + "\n");
  });
} catch (error) {
  process.stderr.write("\n" + spec.marker + JSON.stringify({ status: null, signal: null, timed_out: false, error_code: error?.code ?? "ERUNNER" }) + "\n");
}
`;

function runCriterionProcess({ executable, args }, { repo, env, timeoutMs }) {
  const marker = `WORKLOOP_RUNNER_${randomUUID()}_`;
  const result = spawnSync(process.execPath, ["--input-type=commonjs", "-e", CRITERION_RUNNER_SOURCE], {
    cwd: repo,
    encoding: "utf8",
    env,
    input: JSON.stringify({ executable, args, cwd: repo, env, timeout_ms: timeoutMs, marker }),
...
function runCriterionSource(source, repo, timeoutSeconds = CRITERION_TIMEOUT_SECONDS, protocol = "binary", {
  ...
  const executionTimeoutMs = Math.max(1, Math.min(configuredTimeoutMs, deadlineTimeoutMs));
  let result;
  let snapshotFailure = before.error ?? null;
  if (snapshotFailure) {
    result = { status: null, signal: null, error: snapshotFailure, stdout: "", stderr: "" };
  } else if (Number.isSafeInteger(deadlineEpochMs) && deadlineTimeoutMs <= 0) {
    ...
    try {
      ...
      result = runCriterionProcess(invocation, { repo, env: spawnEnv(), timeoutMs: executionTimeoutMs });
    } catch (error) {
    ...
    result = runCriterionProcess(invocation, { repo, env, timeoutMs: executionTimeoutMs });
