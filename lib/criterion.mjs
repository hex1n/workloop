...
import { CRITERION_TIMEOUT_SECONDS, STATE_DIR, fnv1aHex, globToRegExp, localTimestamp, outputTail } from "./prims.mjs";
...
function windowsCommandResolvable(executable, repo, env) {
  ...
  if (spawnSync("where.exe", [executable], options).status === 0) return true;
  const commandProcessor = env.COMSPEC || path.join(env.SystemRoot || env.windir || "C:\\Windows", "System32", "cmd.exe");
  return spawnSync(commandProcessor, ["/d", "/s", "/c", `help ${executable}`], options).status === 0;
}
...
function mapExecution(result, protocol, timeoutSeconds, options = {}) {
  ...
  return {
    ...
    execution: {
      ...
      output_tail: outputTail(String(result.stdout ?? "") + String(result.stderr ?? "")),
...
function criterionMetadata({ source, protocol, timeoutSeconds, subjects = [], repo }) {
  const declared_inputs = source.kind === "file"
    ? [{ path: source.value, hash: fnv1aHex(fs.readFileSync(path.resolve(repo, source.value), "latin1")) }]
    : [];
  ...
  return {
    ...
    criterion_input_fingerprint: declared_inputs.length ? fnv1aHex(JSON.stringify(declared_inputs)) : null,
...
function criterionDrift(criterion, repo) {
  ...
  for (const input of criterion.declared_inputs ?? []) {
    ...
    let hash = "missing";
    try { hash = fnv1aHex(fs.readFileSync(path.resolve(repo, input.path), "latin1")); } catch { /* missing */ }
