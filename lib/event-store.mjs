...
function validateDescriptor(value, descriptor, { allowLegacy = false } = {}) {
  if (descriptor.startsWith("null|")) {
    if (value === null) return;
    return validateDescriptor(value, descriptor.slice(5), { allowLegacy });
  }
  if (descriptor.startsWith("object:")) {
    ...
    exactKeys(value, legacyCriterion ? expected.filter((field) => field !== "authored_by") : expected, "UNKNOWN_EVENT_FIELD");
    for (const [field, child] of Object.entries(contract)) if (!(legacyCriterion && field === "authored_by")) validateDescriptor(value[field], child, { allowLegacy });
    return;
  }
  ...
  if (array) {
    ...
    for (const child of value) validateDescriptor(child, array[2], { allowLegacy });
    return;
  }
...
function validateEventPayload(kind, payload, { allowLegacy = false } = {}) {
  ...
  exactKeys(payload, legacyAmend ? expected.filter((field) => field !== "artifact_revision") : expected, "UNKNOWN_EVENT_FIELD");
  for (const [field, descriptor] of Object.entries(contract)) if (!(legacyAmend && field === "artifact_revision")) validateDescriptor(payload[field], descriptor, { allowLegacy });
...
function validateEvent(event, { allowLegacy = false } = {}) {
  ...
  const payloadFields = V3_EVENT_PAYLOAD_FIELDS[event.kind];
  ...
  validateEventPayload(event.kind, event.payload, { allowLegacy });
