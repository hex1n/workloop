...
const NESTED_CONTRACTS = Object.freeze({
  ...
  "artifact-entry": { path: "repo-relative-path", kind: "enum:file|symlink", hash: "sha256-digest" },
  "artifact-entry-state": { kind: "enum:file|symlink", hash: "sha256-digest" },
  "artifact-change": { path: "repo-relative-path", before: "null|object:artifact-entry-state", after: "null|object:artifact-entry-state" },
  "artifact-checkpoint": { checkpoint_id: "sha256-digest", captured_at_ms: "non-negative-safe-integer", entries: "array<object:artifact-entry>" },
  "coverage-basis": {
    history_requirement: "enum:artifact_only|complete", artifact_state: "enum:full|unknown",
    mutation_history: "enum:full|partial|unknown", prewrite_enforcement: "enum:full|partial|unknown",
  },
  ...
  "review-record-v2": {
    review_id: "uuid", criterion_generation_id: "uuid", reviewed_task_revision: "positive-safe-integer",
    reviewed_artifact_revision: "non-negative-safe-integer", reviewed_evidence_revision: "non-negative-safe-integer",
    level: "enum:fresh_context|second_model", reviewer: "non-empty-string",
    blocking_findings_count: "non-negative-safe-integer", advisory_findings_count: "non-negative-safe-integer",
    reviewed_at: "utc-iso-milliseconds", acting_session: "null|non-empty-string",
  },
});
...
const PAYLOAD_CONTRACTS = Object.freeze({
  ...
  artifact_reconciled: {
    ...
    captured_at_ms: "non-negative-safe-integer",
  ...
  operation_intent_recorded: {
    ...
    policy_mode: "enum:observe|nudge|deny", policy_disposition: "enum:conformant|deviation",
    policy_reasons: "array<enum:control_plane|foreign_session|suspended|host_approval_unavailable|host_approval_bypassed|git_grant_missing|safety_grant_missing|budget_exhausted|history_unavailable|history_gap|external_target|outside_envelope>",
    session_relation: "enum:owner|foreign|unbound",
  },
});

const PAYLOAD_CONTRACTS_V2 = Object.freeze({
  task_opened: {
    ...PAYLOAD_CONTRACTS.task_opened,
    runtime_contract: "literal:6", artifact_baseline: "object:artifact-checkpoint", coverage_basis: "object:coverage-basis",
  },
  write_authorized: {
    operation_id: "non-empty-string", tool_family: "non-empty-string", declared_targets: "non-empty-array<repo-relative-path>",
    target_coverage: "enum:exact|partial|unknown", host_profile: "non-empty-string", receipt_expectation: "enum:post|failure|reconcile|unknown",
  ...
  task_amended: {
    ...PAYLOAD_CONTRACTS.task_amended,
    history_requirement: "null|enum:artifact_only|complete",
  },
  review_recorded: { record: "object:review-record-v2" },
});

const PAYLOAD_CONTRACTS_V3 = Object.freeze({
  task_opened: {
    ...PAYLOAD_CONTRACTS.task_opened,
    runtime_contract: "literal:7", artifact_baseline: "object:artifact-checkpoint", coverage_basis: "object:coverage-basis",
  },
});
...
function validateDescriptor(value, descriptor, { allowLegacy = false, path: fieldPath = null } = {}) {
  ...
  if (descriptor.startsWith("literal:")) {
    const literal = descriptor.slice("literal:".length);
    valid = value === (/^-?\d+$/u.test(literal) ? Number(literal) : literal);
  } else if (descriptor === "uuid") valid = isUuidV4(value);
...
function validateEventPayload(kind, payload, { allowLegacy = false, payloadVersion = 1 } = {}) {
  const registry = payloadVersion === 1 ? PAYLOAD_CONTRACTS : payloadVersion === 2 ? PAYLOAD_CONTRACTS_V2 : PAYLOAD_CONTRACTS_V3;
  const contract = registry[kind];
