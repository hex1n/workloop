...
const NESTED_CONTRACTS = Object.freeze({
  ...
  "criterion-definition": {
    source: "object:criterion-source", protocol: "enum:binary|tri-state", timeout_seconds: "positive-safe-integer",
...
const PAYLOAD_CONTRACTS = Object.freeze({
  ...
  criterion_observed: { observation: "object:observation", attempt_id: "null|uuid", signature: "null|non-empty-string", failure_summary: "string:max-160-utf8-bytes", drift: "array<repo-relative-path>", source: "enum:open|stop|achieve" },
  ...
  task_amended: {
    ...
    criterion: "null|object:criterion-definition", policy: "null|object:policy", policy_rationale: "null|non-empty-string", generation_id: "null|uuid",
  },
...
function eventStoreError(code) {
  return Object.assign(new Error(ERROR_MESSAGES[code] ?? code), { code });
}
...
function verifyEventStoreSourceCursor(repo, sourceCursor, { fsOps = fs } = {}) {
  if (
    !isPlainObject(sourceCursor) ||
    JSON.stringify(Object.keys(sourceCursor).sort()) !== JSON.stringify([...SOURCE_CURSOR_FIELDS].sort()) ||
    ...
    !isDigest(sourceCursor.record_digest) || !isDigest(sourceCursor.event_id) ||
...
function exactKeys(value, expected, code) {
  if (!isPlainObject(value) || JSON.stringify(Object.keys(value).sort()) !== JSON.stringify([...expected].sort())) throw eventStoreError(code);
}

function validateDescriptor(value, descriptor) {
  if (descriptor.startsWith("null|")) {
    if (value === null) return;
    return validateDescriptor(value, descriptor.slice(5));
  }
  if (descriptor.startsWith("object:")) {
    const contract = NESTED_CONTRACTS[descriptor.slice(7)];
    if (!contract) throw new Error(`unknown persisted descriptor: ${descriptor}`);
    exactKeys(value, Object.keys(contract), "UNKNOWN_EVENT_FIELD");
    for (const [field, child] of Object.entries(contract)) validateDescriptor(value[field], child);
    return;
  }
  const array = descriptor.match(/^(non-empty-)?array<(.+)>$/);
  if (array) {
    if (!Array.isArray(value) || (array[1] && value.length === 0)) throw eventStoreError("UNKNOWN_EVENT_FIELD");
    for (const child of value) validateDescriptor(child, array[2]);
    return;
  }
  if (descriptor.startsWith("enum:")) {
    if (!descriptor.slice(5).split("|").includes(value)) throw eventStoreError("UNKNOWN_EVENT_FIELD");
    return;
  }
  let valid = false;
  if (descriptor === "uuid") valid = isUuid(value);
  else if (descriptor === "sha256-digest") valid = isDigest(value);
  ...
  if (!valid) throw eventStoreError("UNKNOWN_EVENT_FIELD");
}

function validateEventPayload(kind, payload) {
  const contract = PAYLOAD_CONTRACTS[kind];
  if (!contract) throw eventStoreError("UNKNOWN_EVENT_KIND");
  exactKeys(payload, Object.keys(contract), "UNKNOWN_EVENT_FIELD");
  for (const [field, descriptor] of Object.entries(contract)) validateDescriptor(payload[field], descriptor);
  if (kind === "output_tokens_tallied") {
    if (payload.mode === "baseline" && (payload.output_tokens_delta !== 0 || payload.to_offset < payload.from_offset)) throw eventStoreError("UNKNOWN_EVENT_FIELD");
    if (payload.mode === "increment" && payload.to_offset <= payload.from_offset) throw eventStoreError("UNKNOWN_EVENT_FIELD");
  }
  if (kind === "task_terminal") {
    if (payload.outcome === "not_needed" && payload.evidence === null) throw eventStoreError("UNKNOWN_EVENT_FIELD");
    if (payload.outcome === "abandoned" && payload.reason === null) throw eventStoreError("UNKNOWN_EVENT_FIELD");
  }
  if (kind === "task_amended" && payload.policy !== null && payload.criterion === null && payload.generation_id === null) throw eventStoreError("UNKNOWN_EVENT_FIELD");
}
...
function validateEvent(event) {
  ...
  if (!V3_EVENT_KINDS.includes(event.kind)) throw eventStoreError("UNKNOWN_EVENT_KIND");
  if (event.payload_version !== 1) throw eventStoreError("UNSUPPORTED_PAYLOAD_VERSION");
  if (!isUuid(event.task_id) || !Number.isSafeInteger(event.task_event_sequence) || event.task_event_sequence < 1) throw eventStoreError("TASK_SEQUENCE_GAP");
  exactKeys(event.payload, V3_EVENT_PAYLOAD_FIELDS[event.kind], "UNKNOWN_EVENT_FIELD");
  validateEventPayload(event.kind, event.payload);
  if (!isDigest(event.event_id) || sha256Hex(canonicalJson(eventPreimage(event))) !== event.event_id) throw eventStoreError("CORRUPT_EVENT_DIGEST");
  return event;
}
...
function validateRecord(record, { expectedRepoSequence = null, previousRecordDigest = undefined, taskSequences = null } = {}) {
  ...
  if (record.record_schema_version !== V3_EVENT_RECORD_SCHEMA_VERSION) throw eventStoreError("UNSUPPORTED_RECORD_VERSION");
  if (!isUuid(record.transaction_id) || !(record.command_id === null || (typeof record.command_id === "string" && record.command_id.length > 0))) throw eventStoreError("UNKNOWN_RECORD_FIELD");
  ...
  if (!(record.previous_record_digest === null || isDigest(record.previous_record_digest))) throw eventStoreError("HASH_CHAIN_MISMATCH");
  ...
  if (!isDigest(record.record_digest) || sha256Hex(canonicalJson(recordPreimage(record))) !== record.record_digest) throw eventStoreError("CORRUPT_RECORD_DIGEST");
  return record;
}
...
function eventStorePath(repo) {
  return path.join(path.resolve(repo), ".taskloop", EVENT_STORE_FILE);
}
...
function recoverTornTail(target, bytes, { fsOps = fs, onSeam = null, recoveryEpochMs = Date.now() } = {}) {
  ...
  if (!receipt) {
    ...
    const quarantinePath = path.join(quarantineDirectory, `events-v3-tail-${suffix}.bin`);
    const receiptPath = path.join(quarantineDirectory, `events-v3-tail-${suffix}.json`);
    ...
    durableReplace(receiptPath, Buffer.from(`${canonicalJson(receipt)}\n`, "utf8"), fsOps);
...
function parseCompleteRecords(bytes, { firstRepoSequence = 1, previousRecordDigest = null, taskSequences = {} } = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    records.push(record);
    for (const event of record.events) events.push({ ...clone(event), at: record.occurred_at, atEpochMs: record.occurred_at_epoch_ms });
...
function readEventStoreTail(repo, cursor, { fsOps = fs, onSeam = null, recoveryEpochMs = Date.now(), recoverTornTail: mayRecoverTornTail = false } = {}) {
  if (
    ...
    !(cursor.last_record_digest === null || isDigest(cursor.last_record_digest)) || !isPlainObject(cursor.task_sequences)
