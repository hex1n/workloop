...
import { STATE_DIR, foldCasePath, isPlainObject, localTimestamp, repoRelative } from "./prims.mjs";
...
function observeUntracked({ payload, sessionId = null, foreign = false, mode = "nudge", repo, writeShaped, writeTargets, scriptPath, now = Date.now() }) {
  ...
  const bucket = session ? state.sessions[session] : null;
  const prior = bucket?.files ?? [];
  ...
  for (const raw of writeTargets) {
    ...
    if (rel) {
      const folded = foldCasePath(rel);
      known.add(folded);
      inside.push(folded);
    }
  }
  const files = [...known].sort();
  const desiredGate = mode;
  // This is a compact transition ledger, not a decision-attempt counter:
  // repeated denials in the same gate state deliberately do not emit a row.
  const changed = inside.some((rel) => !prior.includes(rel)) || bucket?.gate !== desiredGate;
  const seq = (Number.isSafeInteger(bucket?.seq) ? bucket.seq : 0) + (changed ? 1 : 0);
  if (session) {
    state.sessions[session] = { files, ts: localTimestamp(now), seq, gate: desiredGate };
    try {
      fs.mkdirSync(path.join(repo, STATE_DIR), { recursive: true });
      // Atomic replace: concurrent hooks read this file, so never expose a torn write.
      const temporary = `${untrackedPath(repo)}.${process.pid}.tmp`;
      fs.writeFileSync(temporary, JSON.stringify(state, null, 2) + "\n", "utf8");
      fs.renameSync(temporary, untrackedPath(repo));
    } catch {
      /* the nudge must never break the tool call */
    }
  }
  const openTemplate = foreign
    ? "  parallel work: use a separate worktree; to continue this task: workloop join --reason \"<handoff reason>\""
    :
    "  source the done-when from the requester first — never invent a check to pass this gate; with no honest criterion, ask instead of opening.\n" +
    "  discipline lives in the workloop skill (workloop/SKILL.md in your skills root): criterion sourcing, policies, closure.\n" +
  ...
  const evidence = changed ? { gate_seq: seq, gate: desiredGate, targets_parsed: [...new Set(inside)] } : null;
  if (mode === "deny" && session && inside.length && files.length >= 2) {
    return {
      kind: "deny",
      evidence,
      message:
        `workloop: untracked multi-file work this session (${files.join(", ")}). ` +
  ...
  if (mode === "observe") return { kind: "allow", evidence };
  return {
    kind: "notice",
    evidence,
    message:
      (foreign ? "workloop: foreign session write outside the task envelope; use a separate worktree for parallel work or join to continue this task:\n" : "workloop: no open task — single-file so far; if this is landing wider work, open a task before the next file:\n") +
      openTemplate,
  };
}
