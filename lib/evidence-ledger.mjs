...
import { STATE_DIR, isPlainObject } from "./prims.mjs";

const EVIDENCE_FILE = "untracked-observations-v1.jsonl";
const EVIDENCE_SEQUENCE_FILE = "evidence-sequences-v1.json";
const EVIDENCE_LOCK_DIR = "evidence-ledger.lock";
const EVIDENCE_LOSS_DIR = "evidence-loss-v1";
...
const EVIDENCE_LOCK_WAIT = new Int32Array(new SharedArrayBuffer(4));
...
function acquireEvidenceLock(repo, now = Date.now) {
  const target = lockPath(repo);
  const deadline = now() + EVIDENCE_LOCK_TIMEOUT_MS;
  for (;;) {
    try {
      fs.mkdirSync(target);
      try { fs.writeFileSync(path.join(target, "owner.json"), JSON.stringify({ pid: process.pid, at: new Date(now()).toISOString() }) + "\n", "utf8"); }
      catch (error) {
        try { fs.rmSync(target, { recursive: true, force: true }); } catch { /* preserve the original acquisition error */ }
        throw error;
      }
      return () => { try { fs.rmSync(target, { recursive: true, force: true }); } catch { /* telemetry lock cleanup degrades open */ } };
    } catch (error) {
      if (error?.code !== "EEXIST") throw error;
      try {
        if (now() - fs.statSync(target).mtimeMs > EVIDENCE_LOCK_STALE_MS) {
      ...
      if (now() >= deadline) throw new Error("evidence ledger lock timeout");
      Atomics.wait(EVIDENCE_LOCK_WAIT, 0, 0, 5);
    }
  }
}
...
function appendEvidence(repo, record) {
  let release = null; let sequenceReserved = false;
  try {
    ...
    release = acquireEvidenceLock(repo);
...
function foldEvidence(rows) {
  ...
  const corruptRows = rows.integrity?.corrupt_rows ?? 0;
  const unknownSchemaRows = rows.integrity?.unknown_schema_rows ?? 0;
  const knownLostRows = rows.integrity?.known_lost_rows ?? 0;
